...
function PLUGIN:ChatBoxTextChanged(previousText, newText)
	...
	if (string.sub(newText, 1, string.len(prefix) + 2) == prefix.."r ") then
	...
	elseif (string.sub(newText, 1, string.len(prefix) + 3) == prefix.."rw ") then
		...
		return true;
	elseif (string.sub(newText, 1, string.len(prefix) + 3) == prefix.."ry ") then
		if (string.sub(previousText, 1, string.len(prefix) + 3) != prefix.."ry ") then
			RunConsoleCommand("cwTypingStart", "r");
		end;
