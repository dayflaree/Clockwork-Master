...
local COMMAND = Clockwork.command:New("Spa");
COMMAND.tip = "Speak in Spanish.";
	...
	if (Clockwork.player:HasFlags(player, "P")) then
		...
		for k,v in pairs(_player.GetAll()) do
			...
			if (math.floor(distance) <= Clockwork.config:Get("talk_radius"):Get()) then
				...
				Clockwork.chatBox:SendColored(v, Color(102, 205, 170, 255), "[Spanish] "..player:Name()..": "..speech)
			else
				Clockwork.chatBox:SendColored(v, Color(102, 205, 170, 255), player:Name().." speaks in Spanish, which you do not understand.")
				end;
			end;
		end;
	else
		Clockwork.player:Notify(player, "You do not speak Spanish!")
	end;
end;
