...
if (SERVER) then
	...
	function ENT:IsFriend( entity )
		...
		elseif (entity:IsPlayer()) then
			return (Schema:PlayerIsCombine(entity) or entity:GetFaction() == FACTION_PROSELYTE);
		else
			return false;
		end;
	end;
	...
	function PLUGIN:GravGunOnPickedUp(player, ent)
		if (ent:GetClass() == "cw_hopper") then
	...
	function PLUGIN:GravGunOnDropped(player, ent)
		if (ent:GetClass() == "cw_hopper" and ent:GetState() != STATE_TRIGGERED) then
	...
	function PLUGIN:GravGunPunt(player, ent)
		if (ent:GetClass() == "cw_hopper" and ent:GetState() == STATE_CAPTIVE) then
		...
		if (IsValid(player) and IsValid(player:GetActiveWeapon())) then
			if (player:GetActiveWeapon():GetClass() == "weapon_physcannon") then
				if (key == IN_ATTACK2) then
					...
					if (IsValid(eyeEnt) and eyeEnt:GetClass() == "cw_hopper") then
