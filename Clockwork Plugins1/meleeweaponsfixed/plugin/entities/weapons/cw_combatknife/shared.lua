			...
			if (IsValid(trace.Entity)) then
				...
				if (trace.Entity:IsPlayer() or trace.Entity:IsNPC()) then
					...
					timer.Simple(FrameTime() * 0.5, function()
						if (IsValid(trace.Entity)) then
							trace.Entity:TakeDamageInfo(Clockwork.kernel:FakeDamageInfo(self.Primary.Damage + strength, self, self.Owner, trace.HitPos, DMG_SLASH, 2));
						end;
					end);
					...
					if (IsValid(physicsObject)) then
						...
						if (!player) then
							timer.Simple(FrameTime() * 0.5, function()
								if (IsValid(trace.Entity)) then
									trace.Entity:TakeDamageInfo(Clockwork.kernel:FakeDamageInfo((self.Primary.Damage * 2) + strength, self, self.Owner, trace.HitPos, DMG_SLASH, 2));
								end;
							end);
...
function SWEP:SecondaryAttack()
	local secondaryDelay = self.Secondary.Delay;
	...
	if (SERVER) then
		...
		if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 56) then
			if (IsValid(trace.Entity)) then
				local player = Clockwork.entity:GetPlayer(trace.Entity);
				local strength = Clockwork.attributes:Fraction(self.Owner, ATB_STRENGTH, 10, 5);
				...
				if (trace.Entity:IsPlayer() or trace.Entity:IsNPC()) then
					...
					timer.Simple(FrameTime() * 0.5, function()
						if (IsValid(trace.Entity)) then
							trace.Entity:TakeDamageInfo(Clockwork.kernel:FakeDamageInfo(self.Secondary.Damage + strength, self, self.Owner, trace.HitPos, DMG_SLASH, 2));
						end;
					end);
