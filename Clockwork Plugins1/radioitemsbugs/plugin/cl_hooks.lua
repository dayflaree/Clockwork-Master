...
function PLUGIN:GetEntityMenuOptions(entity, options)
	...
	if (entity:GetClass() == "cw_bug") then
		if (!entity:IsOff()) then
			options["Disable"] = "cw_radioToggle";
		else
			options["Enable"] = "cw_radioToggle";
		end;
		
		--[[options["Set Frequency"] = function()
			Derma_StringRequest("Frequency", "<:: ENTER LISTENING FREQUENCY", frequency, function(text)
				if (IsValid(entity)) then
					Clockwork.entity:ForceMenuOption(entity, "Set Frequency", text);
				end;
			end);
