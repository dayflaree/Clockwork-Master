...
function SWEP:IdleNow()
	...
	if( type( self.IdleAct ) == "string" ) then
		
		--self:SendWeaponAnim( self:GetSequenceActivity( self:LookupSequence( self.IdleAct ) ) );
...
function SWEP:DrawWorldModel()
	...
	if( self.Owner:PlayerClass() == PLAYERCLASS_SUPERMUTANT ) then
	
		local handPos,handAng = self.Owner:GetBonePosition( self.Owner:LookupBone( "Bip01 R Hand" ) );
		
		if( handPos ) then
			
			self:SetRenderOrigin( handPos + handAng:Forward() * 15 );
			self:SetRenderAngles( handAng );
