...
local function nSetAdmin( len )
	...
	GAMEMODE:AddChat( { CB_ALL, CB_OOC }, "Infected.ChatNormal", COLOR_NOTIFY, targ .. "'s rank was set to " .. r .. " by " .. ply:Nick() .. "." );
	
end
net.Receive( "nSetAdmin", nSetAdmin );

local function nTakeAdmin( len )

	local targ = net.ReadString();
	local ply = net.ReadEntity();
	
	
	GAMEMODE:AddChat( { CB_ALL, CB_OOC }, "Infected.ChatNormal", COLOR_NOTIFY, targ .. "'s rank was taken away by " .. ply:Nick() .. "." );
	
end
net.Receive( "nTakeAdmin", nTakeAdmin );
...
local function nSetPlayerModel( len )
	...
	if( targ == LocalPlayer() ) then
		...
		GAMEMODE:AddChat( { CB_ALL, CB_OOC }, "Infected.ChatNormal", COLOR_NOTIFY, ply:Nick() .. " set your model to \"" .. val .. "\"." );
		
	else
		
		GAMEMODE:AddChat( { CB_ALL, CB_OOC }, "Infected.ChatNormal", COLOR_NOTIFY, "You set " .. targ:Nick() .. "'s model to \"" .. val .. "\"." );
		
	end
	
end
net.Receive( "nSetPlayerModel", nSetPlayerModel );
...
local function nItemsList( len )
	...
	for _, v in pairs( GAMEMODE.MetaItems ) do
		...
		if( v.Class == "caps" ) then continue end
