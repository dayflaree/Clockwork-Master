...
function GM:PlayerSpawnRagdoll( ply, model )
	...
	if( !IsValid( ply ) ) then return end -- whenever the server would start, cac would analyze this file and it would error out.
...
function GM:CanTool( ply, tr, tool )
	...
	if( ply:SteamID() == STEAMID_RUSTY and tr.Entity and tr.Entity:IsValid() and tr.Entity:IsPlayer() ) then
	...
	if( tr.Entity:IsPlayer() and !ply:IsAdmin() ) then return false end
	if( tr.Entity:IsNPC() ) then return false end
	
	if( !ply:ToolTrust() ) then return false end
	
	if( table.HasValue( self.ToolTrustBlacklist, tool ) and !ply:IsAdmin() ) then return false end
	
	if( SERVER ) then
	
		if( tr.Entity:GetCreator() != ply and ( tr.Entity:GetClass() != "worldspawn" ) ) then return false end
