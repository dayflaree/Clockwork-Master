			...
			if (ASS_RunPluginFunction( "AllowMute", true, PLAYER, TO_RECIEVE, ENABLE )) then
				...
				if (ENABLE) then
					for k, v in pairs(player.GetAll()) do
						umsg.Start("ass_mute_player", v)
							umsg.Entity( TO_RECIEVE )
						umsg.End()
					end
...
if (CLIENT) then
	function MutePlayer( um )
		local obj = um:ReadEntity()
		
		LocalPlayer():SetMuted( obj )

	end
	usermessage.Hook( "ass_mute_player", MutePlayer)
