	...
	for _,ply in pairs(OCRP_Voting) do
		if ply:IsValid() then
			...
			GUI_Player.DoClick = function()
								RunConsoleCommand("OCRP_Vote",ply:EntIndex())
								GUI_Vote_Frame:Remove()
							end
...
concommand.Add("OCRP_VoteMenu",function(ply,cmd,args) GUI_VoteMenu() end)
...
function GUI_MayorMenu()
	...
	GUI_Property_Sheet:AddSheet( "Economy", GUI_Rebuild_Eco(GUI_Mayor_Frame) , "gui/silkicons/star", true, true, "Manage Salaries and the economy" )									
	GUI_Property_Sheet:AddSheet( "Item Management", GUI_Rebuild_Ill(GUI_Mayor_Frame) , "gui/silkicons/box", true, true, "Legal/Illegalize items" )		
	GUI_Property_Sheet:AddSheet( "Player Management", GUI_Rebuild_Ply(GUI_Mayor_Frame) , "gui/silkicons/user", true, true, "Demote/Promote/Warrent" )	
...
concommand.Add("OCRP_MayorMenu",function(ply,cmd,args) if LocalPlayer():Team() == CLASS_Mayor then GUI_MayorMenu() end end)
...
function GUI_Rebuild_Eco(parent)
	...
	local GUI_Eco_Points = vgui.Create("DBevel")
	...
	GUI_Tax_slider.ValueChanged = function()
		RunConsoleCommand("OCRP_TaxUpdate",math.Round(math.Clamp(math.abs(GUI_Tax_slider:GetValue()),0,10)))
	end
	...
	GUI_Police_Paycheck.ValueChanged = function()
		RunConsoleCommand("OCRP_PoliceIncome_Update",math.Round(math.Clamp(math.abs(GUI_Police_Paycheck:GetValue()),0,10)))
		GAMEMODE.CL_PolicePayCheck = math.Clamp(tonumber(GUI_Police_Paycheck:GetValue()),0,10)
	end
	...
	GUI_Police_Donate.DoClick = function()
									RunConsoleCommand("OCRP_Mayor_Donate_Police",math.Clamp(GUI_Police_Donation:GetValue(),0,GAMEMODE.CL_MayorMoney))
								end	
...
function GUI_Rebuild_Ill(parent)
	...
	GUI_Illegalize.DoClick = function()
									for item1,bool in pairs(IllegalItems) do
										RunConsoleCommand("OCRP_IllegalizeItem",item1,tostring(bool))
									end
								end
...
function GUI_Rebuild_Ply(parent)
	...
	GUI_Player_List:AddColumn("Warrent Type")
	...
	for _,ply in pairs(player.GetAll()) do
		if ply != LocalPlayer() then 
			...
			GUI_Player_List:AddLine(ply:Nick(),warrented,status)
		end 
	end
	GUI_Player_List.OnRowSelected = function() 
		local GUI_Player_Options = DermaMenu()
		GUI_Player_Options:SetPos(gui.MousePos())
		GUI_Player_Options:AddOption("Search Warrent : "..GUI_Player_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Player_List:GetSelected()[1]:GetValue(1),1) end)
		GUI_Player_Options:AddOption("Arrest Warrent : "..GUI_Player_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Player_List:GetSelected()[1]:GetValue(1),2) end)
		GUI_Player_Options:AddOption("Remove all Warrents",function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Player_List:GetSelected()[1]:GetValue(1),0) end)
		if GUI_Player_List:GetSelected()[1]:GetValue(3) != "Citizen" then
			GUI_Player_Options:AddOption("Demote "..GUI_Player_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_DemotePlayer",GUI_Player_List:GetSelected()[1]:GetValue(1)) end)
			if GUI_Player_List:GetSelected()[1]:GetValue(3) == "Police" then
				GUI_Player_Options:AddOption("Promote To Police Chief - $250",function() RunConsoleCommand("OCRP_Hire_Police_Chief",GUI_Player_List:GetSelected()[1]:GetValue(1)) end)
			end
		end
	...
	for _,data in pairs(GAMEMODE.OCRP_Economy_Events[event].Choices) do
		...
		GUI_Choice.DoClick = function()
									if data.Price != nil then
										...
										RunConsoleCommand("OCRP_Mayor_Choice",data.Name)
...
concommand.Add("OCRP_EventMenu",function(ply,cmd,args) GUI_MayorEventMenu() end)
...
function GUI_Mayor_Catalog()
	...
	for item,data in pairs(GAMEMODE.Mayor_Items) do
		...
		GUI_Cata_Item_Buy.DoClick = function()
										RunConsoleCommand("OCRP_Mayor_CreateObj",item)
									end
