...
function GUI_ChiefMenu()
	...
	GUI_Property_Sheet:AddSheet( "Player Management", GUI_Rebuild_Police_Ply(GUI_Chief_Frame) , "gui/silkicons/user", true, true, "Demote/Warrent" )	
...
concommand.Add("OCRP_ChiefMenu",function(ply,cmd,args) if LocalPlayer():Team() == CLASS_CHIEF then GUI_ChiefMenu() end end)
...
function GUI_Rebuild_Police_Locker(parent)
	...
	GUI_Max_p228_Ammo_Out.ValueChanged = function()
		RunConsoleCommand("OCRP_PoliceMax_Update","item_ammo_cop",math.Round(math.Clamp(math.abs(GUI_Max_p228_Ammo_Out:GetValue()),8,64)))
	...
	GUI_Max_buckshot_Ammo_Out.ValueChanged = function()
		RunConsoleCommand("OCRP_PoliceMax_Update","item_ammo_riot",math.Round(math.Clamp(math.abs(GUI_Max_buckshot_Ammo_Out:GetValue()),8,24)))
	...
	GUI_Max_ump_Ammo_Out.ValueChanged = function()
		RunConsoleCommand("OCRP_PoliceMax_Update","item_ammo_ump",math.Round(math.Clamp(math.abs(GUI_Max_ump_Ammo_Out:GetValue()),25,125)))
	...
	GUI_Drop_Button.DoClick = function()
								for _,data in pairs(GAMEMODE.Locker_Items) do
									if data.Item == item then
										if GAMEMODE.CL_PoliceMoney >= data.Price then
											RunConsoleCommand("OCRP_Chief_Supply_Locker",item,math.abs(tonumber(GUI_Amount_slider:GetValue())))
										end
									end
								end
...
function GUI_Rebuild_Police_Ply(parent)
	...
	GUI_Police_List.OnRowSelected = function() 
		...
		GUI_Player_Options:AddOption("Search Warrent : "..GUI_Police_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Police_List:GetSelected()[1]:GetValue(1),1) end)
		GUI_Player_Options:AddOption("Arrest Warrent : "..GUI_Police_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Police_List:GetSelected()[1]:GetValue(1),2) end)
		GUI_Player_Options:AddOption("Remove all Warrents",function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Police_List:GetSelected()[1]:GetValue(1),0) end)
		GUI_Player_Options:AddOption("Demote "..GUI_Police_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_DemotePlayer",GUI_Police_List:GetSelected()[1]:GetValue(1)) end)
		GUI_Player_Options:AddOption("Upgrade to SWAT - $50 ",function() RunConsoleCommand("OCRP_Swat_Ask",GUI_Police_List:GetSelected()[1]:GetValue(1)) end)
	...
	GUI_Citizen_List:AddColumn("Warrent Type")
	for _,ply in pairs(player.GetAll()) do
		if ply != LocalPlayer() then 
			local status = "Citizen"
			if ply:Team() == CLASS_CITIZEN then
				local warrented = "None"
				if ply:GetNWInt("Warrent") == 1 then
					warrented = "Search Warrent"
				elseif ply:GetNWInt("Warrent") == 1 then
					warrented = "Arrest Warrent"
				end
				GUI_Citizen_List:AddLine(ply:Nick(),warrented)
			end
		end 
	end				
	...
	GUI_Citizen_List.OnRowSelected = function() 
		local GUI_Player_Options = DermaMenu()
		GUI_Player_Options:SetPos(gui.MousePos())
		GUI_Player_Options:AddOption("Search Warrent : "..GUI_Citizen_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Citizen_List:GetSelected()[1]:GetValue(1),1) end)
		GUI_Player_Options:AddOption("Arrest Warrent : "..GUI_Citizen_List:GetSelected()[1]:GetValue(1),function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Citizen_List:GetSelected()[1]:GetValue(1),2) end)
		GUI_Player_Options:AddOption("Remove all Warrents",function() RunConsoleCommand("OCRP_WarrentPlayer",GUI_Citizen_List:GetSelected()[1]:GetValue(1),0) end)
		GUI_Player_Options:AddOption("Cancel",function() end)
		GUI_Player_Options:Open()
	end
...
function GUI_Chief_Catalog()
	...
	local Cata_Icon_ent = ents.Create("prop_physics")
	...
	for item,data in pairs(GAMEMODE.Chief_Items) do
		...
		GUI_Cata_Item_Buy.DoClick = function()
										RunConsoleCommand("OCRP_Chief_CreateObj",item)
									end
...
function GUI_AskSwat()
	...
	GUI_Ask_Exit.DoClick = function()
								RunConsoleCommand("OCRP_Swat_Reply","false",0)
	...
	GUI_Ask_Search.DoClick = function()
							...
							--	GUI_Ask_Frame:Remove()
								...
								local SWAT_Icon_ent = ents.Create("prop_physics")
								...
								SWAT_Icon_ent:Activate()	
									...
									GUI_SWAT_MP5_Icon.DoClick = function()
																	GUI_Ask_Frame:Remove()
																	RunConsoleCommand("OCRP_Swat_Reply","true",1)
																end
									...
									GUI_SWAT_Riot_Icon.DoClick = function()
																	GUI_Ask_Frame:Remove()
																	RunConsoleCommand("OCRP_Swat_Reply","true",0)
																end
