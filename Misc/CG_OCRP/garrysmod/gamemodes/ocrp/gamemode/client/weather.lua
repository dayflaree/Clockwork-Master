...
local function BuildSnowMaterials ( )
	for k, v in pairs(REPLACING_TEXTURES) do
		local defaultSnowMaterial = Material("Nature/snowfloor001a"):GetMaterialTexture("$basetexture");
		...
		if file.Exists("../materials/" .. snowName .. ".vmt") then
		print(snowName)
			newTable[3] = Material(snowName):GetMaterialTexture("$basetexture");
		else
			newTable[3] = defaultSnowMaterial;
		end
		
		newTable[2] = newTable[1]:GetMaterialTexture("$basetexture");
