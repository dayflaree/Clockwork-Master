...
function SWEP:PrimaryAttack()
	...
	local function RamNow()
		if tr.Entity:IsDoor() && tr.Entity:GetNWInt("Owner") > 0 &&  player.GetByID(tr.Entity:GetNWInt("Owner")):IsValid() then
			...
			if player.GetByID(tr.Entity:GetNWInt("Owner")):GetWarrented() > 0 then
				local Random = math.random( 1, 100 )
				if Random > 20 then
					self.Owner:Hint("[OCRP] Unsuccessful attempt at ramming the door.")
	...
	if !tr.Entity:IsDoor() then
		...
		if SERVER then 
			if tr.Entity:GetNWInt("Owner") > 0 && player.GetByID(tr.Entity:GetNWInt("Owner")):GetWarrented() > 0 && tr.Entity:GetClass() == "item_base" then 
