...
function SWEP:Deploy()
	...
	if SERVER then
		...
		if self.Owner:Alive() then
			timer.Simple(0.1,function() if self.Owner:IsValid() then self.Owner:DrawViewModel(false) end end)
		end
	end
	return true
end
...
function SWEP:SecondaryAttack()
	...
	if !self.Hidden then 
		...
		timer.Simple(0.25,function()  self:SetWeaponHoldType(self.HoldType) end)
		if SERVER then
...
function SWEP:PrimaryAttack()
	...
	if !self.Hidden then
		if self.Owner:GetNWInt("Energy") < 2 then return end
		self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
		self.Owner:SetAnimation(PLAYER_ATTACK1)
		self.Weapon:EmitSound("npc/vort/claw_swing" .. math.random(1, 2) .. ".wav");
		if SERVER then
			self.Owner:SetNWInt("Energy",self.Owner:GetNWInt("Energy") - 2)
		end
