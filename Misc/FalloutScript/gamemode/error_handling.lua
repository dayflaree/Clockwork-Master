...
function ErrorHandler( func, args, ret, errcode )
	...
	LEMON.CallHook( "ErrorHandler", func, args, ret, errcode);
	
	local s = func .. "() failed: " .. func .. "(" .. table.concat( args, "," ) .. ") failed with error " .. errcode .. ": " .. LEMON.NilFix(ErrorCodes[errcode], "Invalid Error Code");
	
	--LEMON.DayLog( "errors.txt", s);
	print(s);
