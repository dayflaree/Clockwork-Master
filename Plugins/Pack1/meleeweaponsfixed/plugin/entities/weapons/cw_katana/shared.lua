				...
				if (trace.Entity:IsPlayer() or trace.Entity:IsNPC()) then
					...
					timer.Simple(FrameTime() * 0.5, function()
						if (IsValid(trace.Entity)) then
							...
							if trace.Entity:Health() <= 0 and Clockwork.player:GetRagdollEntity(player) and trace.Entity:TranslatePhysBoneToBone(trace.PhysicsBone) == trace.Entity:LookupBone("ValveBiped.Bip01_Head1") then
								local ragdoll = Clockwork.player:GetRagdollEntity(player);
								ragdoll:ManipulateBoneScale(ragdoll:LookupBone("ValveBiped.Bip01_Head1"), Vector(0.0001,0.0001,0.0001));
							end;
						end;
					end);
