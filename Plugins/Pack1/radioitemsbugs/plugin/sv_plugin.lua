...
-- A function to load the bugs.
function PLUGIN:LoadBugs()
	local radios = Clockwork.kernel:RestoreSchemaData( "plugins/bugs/"..game.GetMap() );
...
-- A function to save the bugs.
function PLUGIN:SaveBugs()
	local radios = {};
	
	for k, v in pairs(ents.FindByClass("cw_bug")) do
		local physicsObject = v:GetPhysicsObject();
		local moveable;
	...
	Clockwork.kernel:SaveSchemaData("plugins/bugs/"..game.GetMap(), radios);
end;
