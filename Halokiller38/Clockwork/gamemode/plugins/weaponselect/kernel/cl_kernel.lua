...
Clockwork.config:AddToSystem("weapon_selection_multi", "Should the weapon selection be hidden if the player has only one weapon.");
...
function PLUGIN:DrawWeaponInformation(itemTable, weapon, x, y, alpha)
	...
	or weapon.Instructions != ""))) then
		...
		Clockwork:OverrideMarkupDraw(weapon.InfoMarkup);
		...
		if (info.drawBackground) then
			Clockwork:DrawTexturedGradientBox(2, x - 4, realY, 260, weaponMarkupHeight + 8, Color(backgroundColor.r, backgroundColor.g, backgroundColor.b, math.min(backgroundColor.a, alpha)));
		end;
