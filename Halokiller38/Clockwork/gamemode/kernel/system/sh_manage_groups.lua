...
if (CLIENT) then
	...
	function SYSTEM:HasAccess()
		if (!Clockwork.config:Get("use_own_group_system"):Get()) then
			local commandTable = Clockwork.command:Get("PlySetGroup");
	...
	function SYSTEM:OnDisplay(systemPanel, systemForm)
		if (self.groupType == GROUP_USER) then
			...
			if (!self.noRefresh) then
				Clockwork:StartDataStream("SystemGroupGet", {self.groupType, self.groupPage});
			else
				self.noRefresh = nil;
			end;
				...
				if (#self.groupPlayers > 0) then
					for k, v in ipairs(self.groupPlayers) do
						...
						function label.DoClick(button)
							local commandTable = Clockwork.command:Get("PlyDemote");
							
							if (commandTable and Clockwork.player:HasFlags(Clockwork.Client, commandTable.access)) then
								Derma_Query("Are you sure that you want to demote "..v.steamName.."?", "Demote "..v.steamName..".", "Yes", function()
									Clockwork:StartDataStream("SystemGroupDemote", {v.steamID, v.steamName, self.groupType});
								end, "No", function() end);
							end;
						end;
					end;
					...
					if (self.pageCount > 1) then
						...
						if (self.isNext) then
							...
							function nextButton.DoClick(button)
								Clockwork:StartDataStream("SystemGroupGet", {self.groupType, self.groupPage + 1});
							end;
						end;
	...
	Clockwork:HookDataStream("SystemGroupRebuild", function(data)
		local systemTable = Clockwork.system:Get("Manage Groups");
	...
	Clockwork:HookDataStream("SystemGroupGet", function(data)
	...
	Clockwork:HookDataStream("SystemGroupDemote", function(player, data)
		local commandTable = Clockwork.command:Get("PlyDemote");
			...
			if (target) then
				...
				timer.Simple(1, function()
					if (IsValid(player)) then
						Clockwork:StartDataStream(player, "SystemGroupRebuild", true);
					end;
				end);
			else
	...
	Clockwork:HookDataStream("SystemGroupGet", function(player, data)
		if (type(data) != "table") then
			return;
		end;
		...
		if (groupPage) then
			...
			tmysql.query("SELECT * FROM "..playersTable.." WHERE _Schema = \""..schemaFolder.."\" AND _UserGroup = \""..groupName.."\"", function(result)
				...
				if (#sendPlayers > 0) then
					Clockwork:StartDataStream(player, "SystemGroupGet", {
						pageCount = pageCount,
						players = sendPlayers,
					...
					Clockwork:StartDataStream(player, "SystemGroupGet", false);
				end;
			end, 1);
		end;
	end);
