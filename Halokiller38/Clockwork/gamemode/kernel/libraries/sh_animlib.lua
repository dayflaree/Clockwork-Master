...
--[[function GetRelativeBoneAngles(ent, bonename)
	...
	local parent = GetBoneParentID(boneindex)
	if not parent or parent == boneindex then
		return matrix:GetAngle() -- No parent? Just return these angles.
	end

	local parentmatrix = ent:GetBoneMatrix(parentmatrix)
	if not parentmatrix then
		return matrix:GetAngle()
	end

	local parentangles = parentmatrix:GetAngle() - (absoluteangles[string.lower(ent:GetBoneName(parent) or "")] or Angle(0, 0, 0))

	return matrix:GetAngle() - parentangles
end]]
