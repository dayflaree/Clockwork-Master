...
Clockwork.entity = Clockwork:NewLibrary("Entity");
...
function Clockwork.entity:GetDoorPartners(entity)
	...
	for k, v in ipairs(doorEntities) do
...
function Clockwork.entity:CanSeePosition(entity, position, allowance, ignoreEnts)
	local trace = {};
...
function Clockwork.entity:CanSeeNPC(entity, target, allowance, ignoreEnts)
	local trace = {};
...
function Clockwork.entity:CanSeePlayer(entity, target, allowance, ignoreEnts)
...
function Clockwork.entity:CanSeeEntity(entity, target, allowance, ignoreEnts)
	local trace = {};
...
if (CLIENT) then
	...
	function Clockwork.entity:FetchItemData(entity)
		...
		if (curTime > entity.m_iNextFetchItemData) then
			...
			Clockwork:StartDataStream("FetchItemData", entity);
		end;
	end;
	
	Clockwork:HookDataStream("FetchItemData", function(data)
	...
	Clockwork:HookDataStream("FetchItemData", function(player, data)
		if (IsValid(data)) then
			local itemTable = data:GetItemTable();
			local definition = Clockwork.item:GetDefinition(itemTable, true);
			
			Clockwork:StartDataStream(player, "FetchItemData", {
...
if (SERVER) then
	...
	function Clockwork.entity:MakeSafe(entity, physgunProtect, toolProtect, freezeEntity)
		...
		if (IsValid(entity)) then
			...
			if (IsValid(physicsObject)) then
				if (!physicsObject:IsPenetrating()) then
					...
					Clockwork:CreateTimer("CollisionGroup"..index, 1, 1, function()
	...
	function Clockwork.entity:Decay(entity, seconds, Callback)
		...
		Clockwork:CreateTimer("Decay"..index, 1, 0, function()
			...
			if (IsValid(entity)) then
				...
				if (a <= 0) then
					...
					entity:Remove();
					Clockwork:DestroyTimer("Decay"..index);
				else
	...
	function Clockwork.entity:ForceMenuOption(entity, option, arguments)
		Clockwork:StartDataStream("EntityMenuOption", {entity, option, arguments});
	end;
