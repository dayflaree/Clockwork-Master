...
function Clockwork.config.class:IsValid()
	return self.data != nil;
end;

-- A function to query the config.
function Clockwork.config.class:Query(key, failSafe)
...
function Clockwork.config.class:GetBoolean(failSafe)
	if (self.data) then
...
function Clockwork.config.class:GetNumber(failSafe)
	if (self.data) then
...
function Clockwork.config.class:GetString(failSafe)
	if (self.data) then
...
function Clockwork.config.class:GetDefault(failSafe)
	if (self.data) then
...
function Clockwork.config.class:GetNext(failSafe)
...
function Clockwork.config.class:Get(failSafe)
...
function Clockwork.config:SetInitialized(initalized)
	self.cwInitialized = initalized;
end;
...
function Clockwork.config:ShareKey(key)
	local shortCRC = Clockwork:GetShortCRC(key);
...
function Clockwork.config:Import(fileName)
	local data = fileio.Read(Clockwork:SetupFullDirectory(fileName));
...
function Clockwork.config:LoadINI(fileName)
	...
	local function StripQuotes(line)
		return line:gsub("^[\"'](.+)[\"']$", "%1"):Trim();
	end;
	
	for k, v in ipairs(explodedData) do
...
function Clockwork.config:Parse(text)
	for key in string.gmatch(text, "%$(.-)%$") do
		...
		if (value != nil) then
			text = Clockwork:Replace(text, "$"..key.."$", tostring(value));
		end;
	end;
...
if (SERVER) then
	function Clockwork.config:Save(fileName, configTable)
		if (configTable) then
			...
			Clockwork:SaveClockworkData(fileName, config.global);
			Clockwork:SaveSchemaData(fileName, config.schema);
		else
			Clockwork:DeleteClockworkData(fileName);
			Clockwork:DeleteSchemaData(fileName);
		end;
	end;
		...
		if (key) then
			if (self.stored[key]) then
				...
				if (self.stored[key].isShared) then
					...
					Clockwork:StartDataStream(player, "Config", { [key] = value });
				end;
			end;
		else
			local config = {};
			...
			Clockwork:StartDataStream(player, "Config", config);
		end;
	end;
	...
	function Clockwork.config:Load(fileName, loadGlobal)
		...
		elseif (loadGlobal) then
			return Clockwork:RestoreClockworkData(fileName);
		else
			return Clockwork:RestoreSchemaData(fileName);
		end;
	end;
	...
	function Clockwork.config.class:Set(value, map, forceSet, temporary)
		if (map) then
	...
	Clockwork:HookDataStream("ConfigInitialized", function(player, data)
	...
	Clockwork:HookDataStream("Config", function(data)
		...
		if (!Clockwork.config:HasInitialized()) then
			...
			if (IsValid(Clockwork.Client) and !Clockwork.config:HasSentInitialized()) then
				Clockwork:StartDataStream("ConfigInitialized", true);
	...
	function Clockwork.config:AddToSystem(key, help, minimum, maximum, decimals)
		self.system[key] = {
	...
	-- A function to set the config's value.
	function Clockwork.config.class:Set(value)
		if (tostring(value) == "-1.#IND") then
			value = 0;
		end;
