...
Clockwork.animation = Clockwork:NewLibrary("Animation");
...
function Clockwork.animation:GetWeaponHoldType(player, weapon)
	...
	elseif (weaponTable and weaponTable.HoldType) then
		...
		if (act != -1) then
			if (self.convert[act]) then
				return self.convert[act];
			else
				return "fist";
			end;
		else
