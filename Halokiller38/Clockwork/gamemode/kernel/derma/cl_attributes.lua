...
function PANEL:Init()
	...
	self:SetTitle(Clockwork.option:GetKey("name_attributes"));
...
function PANEL:Rebuild()
	...
	if (#categories > 0 or #miscellaneous > 0) then
		...
		local attributesForm = vgui.Create("DForm", self);
			...
			attributesForm:SetName(Clockwork.option:GetKey("name_attributes"));
		self.panelList:AddItem(attributesForm);
		
		for k, v in ipairs(miscellaneous) do
			self.currentAttribute = v[1];
			attributesForm:AddItem(vgui.Create("cwAttributesItem", self)) ;
		end;
...
vgui.Register("cwAttributes", PANEL, "DFrame");

local PANEL = {};

-- Called when the panel is initialized.
function PANEL:Init()
	self.attribute = Clockwork.attribute:Get(self:GetParent().currentAttribute);
	...
	function self.baseBar.Paint(baseBar)
		...
		if (boostData.negative) then
			if (attributes[uniqueID] - boostData.boost < 0) then
				Clockwork:DrawSimpleGradientBox(0, 0, 0, boostData.width, baseBar:GetTall(), hinderColor);
			else
				boostData.width = math.min(boostData.width, width);
				
				Clockwork:DrawSimpleGradientBox(0, 0, 0, math.max(width - boostData.width, 0), baseBar:GetTall(), mainColor);
				Clockwork:DrawSimpleGradientBox(0, math.max(width - boostData.width, 0), 0, boostData.width, baseBar:GetTall(), hinderColor);
			end;
		else
			Clockwork:DrawSimpleGradientBox(0, 0, 0, width, baseBar:GetTall(), mainColor);
			Clockwork:DrawSimpleGradientBox(0, width, 0, math.min(boostData.width, baseBar:GetWide()), baseBar:GetTall(), boostColor);
		end;
	end;
	...
	function self.progressBar.Paint(progressBar)
		...
		Clockwork:DrawSimpleGradientBox(0, 0, 0, progressBar:GetWide(), progressBar:GetTall(), color);
		Clockwork:DrawSimpleGradientBox(0, 0, 0, width, progressBar:GetTall(), progressColor);
	end;
...
function PANEL:Paint()
	...
	Clockwork:DrawSimpleGradientBox(4, x, y, width, height, self:GetBackgroundColor());
