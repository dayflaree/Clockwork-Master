			...
			if (IsValid(trace.Entity)) then
				...
				if (info and Clockwork.plugin:Call("PlayerCanLockEntity", self.Owner, trace.Entity)) then
					...
					if (isNotLocking or isNotUnlocking) then
						...
						Clockwork.player:SetAction(player, "lock", info.duration);
						
						Clockwork.player:EntityConditionTimer(player, target, nil, info.duration, 192, function()
							if (!Clockwork.plugin:Call("PlayerCanLockEntity", player, target)) then
								return false;
							else
							...
							if (success) then
								...
								Clockwork.player:SetAction(player, "lock", false);
							end;
						end);
					end;
				end;
			...
			if (IsValid(trace.Entity)) then
				...
				if (info and Clockwork.plugin:Call("PlayerCanUnlockEntity", self.Owner, trace.Entity)) then
					...
					if (isNotLocking or isNotUnlocking) then
						...
						Clockwork.player:SetAction(player, "unlock", info.duration);
						
						Clockwork.player:EntityConditionTimer(player, target, nil, info.duration, 192, function()
							if (!Clockwork.plugin:Call("PlayerCanUnlockEntity", player, target)) then
								return false;
							else
								return player:Alive() and !player:IsRagdolled() and player:IsUsingKeys();
							end;
							...
							if (success) then
								...
								Clockwork.player:SetAction(player, "unlock", false);
							end;
						end);
					end;
				end;
