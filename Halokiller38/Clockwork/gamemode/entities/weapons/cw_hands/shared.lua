...
if (SERVER) then
	...
	resource.AddFile("models/weapons/v_fists_cs.mdl");
...
SWEP.Instructions = "Primary Fire: Knock. Secondary Fire: Knock.";
SWEP.Contact = "";
...
function SWEP:PunchEntity()
	...
	if (SERVER) then
		...
		if (IsValid(traceLineAttack.Entity)) then
			traceLineAttack.Entity:TakeDamageInfo(Clockwork:FakeDamageInfo(self.Primary.Damage, self, self.Owner, traceLineAttack.HitPos, DMG_CLUB, 1));
		end;
	end;
end;
