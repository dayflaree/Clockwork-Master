...
function ENT:Explode()
	...
	for k, v in pairs(player.GetAll()) do
		...
		if (tonumber(propnum)>0) then
			...
			player.GetByUniqueID(uid):Message("A bigbomb has destroyed " .. tostring(owners[uid]) .. " of your props!")
		end
	end
	...
	if (self.Entity:CheckPropFaggotry()==false) then
		...
		for k, v in pairs(player.GetAll()) do
			...
			if (tonumber(propnum)>0) then
				...
				player.GetByUniqueID(uid):Message("A bigbomb has been planted near " .. tostring(owners[uid]) .. " of your props!")
			end
		end
