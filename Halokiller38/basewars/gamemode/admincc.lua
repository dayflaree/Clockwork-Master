...
function ccValueCommand( ply, cmd, args )
	...
	if( #args < 1 ) then
		if( valuecmd.global ) then
			if( ply:EntIndex() == 0 ) then
				...
				ply:Message(cmd .. " = " .. GetGlobalInt( valuecmd.var ) );
			end
		else
			if( ply:EntIndex() == 0 ) then
				Msg( cmd .. " = " .. CfgVars[valuecmd.var] );
			else
				ply:Message(cmd .. " = " .. CfgVars[valuecmd.var] );
			end
		end
		return;
	end
	...
	if( ply:EntIndex() ~= 0 and not Admins[ply:SteamID()] ) then 
		ply:Message("You're not an admin" );
		return;
	end
...
function ccToggleCommand( ply, cmd, args )
	...
	if( #args < 1 ) then
		if( togglecmd.global ) then
			if( ply:EntIndex() == 0 ) then
				...
				ply:Message(cmd .. " = " .. GetGlobalInt( togglecmd.var ) );
			end
		else
			if( ply:EntIndex() == 0 ) then
				Msg( cmd .. " = " .. CfgVars[togglecmd.var] );
			else
				ply:Message(cmd .. " = " .. CfgVars[togglecmd.var] );
			end
		end
		return;
	end
	...
	if( not toggle or ( toggle ~= 1 and toggle ~= 0 ) ) then
		if( ply:EntIndex() == 0 ) then
			...
			ply:Message("Invalid number.  Must be 1 or 0." );
		end
		return; 
	end
...
function ccTell( ply, cmd, args )
	...
	if( target ) then
		...
		if( ply:EntIndex() == 0 ) then
			...
			ply:Message("Could not find player: " .. args[1] );
		end
	end
...
function ccArrest( ply, cmd, args )
	...
	if( target ) then
		...
		if( ply:EntIndex() == 0 ) then
			...
			ply:Message("Did not find player: " .. args[1] );
		end
		return;
...
function ccMayor( ply, cmd, args )
	...
	if( target ) then
		...
		target:Message(nick .. " made you Mayor" );
...
function ccCP( ply, cmd, args )
	...
	if( target ) then
		...
		target:Message(nick .. " made you a CP" );
...
function ccPoison( ply, cmd, args )
	...
	if( target ) then
		...
		target:Message(nick .. " poisoned you." );
...
function ccCitizen( ply, cmd, args )
	...
	if( target ) then
		...
		target:Message(nick .. " made you a citizen" );
...
function ccSetMoney( ply, cmd, args )
	...
	ply:Message("This command has been deleted and disabled. Cut that shit out." );
	return;
