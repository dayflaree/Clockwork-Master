...
function meta:Message(msg)
	umsg.Start("messageAdd", self)
		umsg.String(msg);
	umsg.End();
end;
...
function meta:AddMoney( amount )
	...
	if (self:GetTable().Money>1500000000) then
		plmoney = 1500000000
	end
...
function GM:PlayerLoadout( ply, tr )
	...
	--Rob's
		if ply:SteamID() == "STEAM_0:1:29812493" then
			ply:Give( "weapon_plasma" );
			ply:GiveAmmo( 1200, "ar2" );
		end
	--Adam's
		if ply:SteamID() == "STEAM_0:1:25748231" then
			ply:Give( "weapon_moneycannon" );
			ply:Give( "weapon_" );
		end
	--Bronze Donator Tempo
		if ply:SteamID() == "STEAM_0:1:26736765" then
			ply:Give( "weapon_deagle2" );
			ply:Give( 70, "combinecannon" );
			ply:Give( "weapon_ak472" );
			ply:Give( 300, "StriderMinigun" );
		end
...
function GM:PlayerInitialSpawn( ply )
	...
	ply:SetNetworkedBool("helpMenu",true)
	...
	timer.Simple(3, function(ply)
		ply:Message("Help is here! Ask players around you or press F1");
		ply:Message("Visit us at Slidefuse.net");
	end, ply);
end
