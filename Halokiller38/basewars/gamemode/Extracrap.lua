...
function BuySuperGenerator( ply )
	...
	local tr = util.TraceLine( trace );
		...
		if(ply:GetTable().maxgenerator >= CfgVars["maxsupergenerator"])then
...
function BuyBomb( ply )
	...
	if( not ply:CanAfford( CfgVars["bigbombcost"] ) ) then
		...
		if(ply:GetTable().maxBigBombs >= CfgVars["bigbombmax"])then
			Notify( ply, 4, 3, "You can only have one Bomb out at a time!" );
			return "";
		end
...
--			end
--		
--			target:Message(nick .. " changed your team to " .. args[2] );
--		end
--	else
--	
--		if( ply:EntIndex() == 0 ) then
