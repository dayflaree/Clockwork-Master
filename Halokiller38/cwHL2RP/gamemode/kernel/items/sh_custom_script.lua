...
if (CLIENT) then
	...
	function ITEM:GetEntityMenuOptions(entity, options)
		...
		options["Customize"] = function()
			...
			Derma_StringRequest("Weight", "What much does this item weigh in kilograms?", "", function(text)
			...
			Derma_StringRequest("Reusable", "Can this item be used more than once (valid answers are 'yes' and 'no')?", "", function(text)
				customData.reusable = (string.lower(text) == "yes");
				
			Derma_StringRequest("Category", "What would you like to set the use category to?", "", function(text)
