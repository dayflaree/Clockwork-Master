			...
			if (!target:GetSharedVar("IsTied")) then
				if (target:GetShootPos():Distance(player:GetShootPos()) <= 192) then
					...
					if (canTie or target:IsRagdolled()) then
						...
						Clockwork.player:Notify(player, "You cannot restrain characters that are facing you!");
						return false;
					end;
...
function ITEM:OnDrop(player, position)
	if (player.cwIsTyingChar) then
		Clockwork.player:Notify(player, "You are currently restraining a character!");
		return false;
	end;
end;
