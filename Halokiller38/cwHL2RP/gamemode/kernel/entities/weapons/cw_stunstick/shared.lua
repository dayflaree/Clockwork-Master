...
function SWEP:DrawWorldModel()
	...
	if ( Clockwork.player:GetWeaponRaised(self.Owner) ) then
		...
		local scale = math.abs(math.sin(curTime) * 8);
		...
		if (attachment and attachment.Pos) then
			cam.Start3D( EyePos(), EyeAngles() );
				...
				render.DrawSprite( attachment.Pos, 8 + scale, 8 + scale, Color(180, 180, 255, 255 ) );
			cam.End3D();
		end;
	end;
end;
					...
					cam.Start3D( EyePos(), EyeAngles() );
						...
						for i = 1, 9 do
							...
							if (attachment.Pos) then
								...
								render.DrawSprite( attachment.Pos, 3, 3, Color(180, 180, 255, 255) );
							end;
						end;
			...
			if ( IsValid(trace.Entity) ) then
				...
				if ( trace.Entity:IsPlayer() ) then
					...
					if (trace.Entity:Health() > 10) then
						trace.Entity:TakeDamageInfo( Clockwork:FakeDamageInfo(25, self, self.Owner, trace.HitPos, DMG_CLUB, 2) );
					end;
