...
function ENT:Initialize()
	...
	self.dummy = ents.Create("cw_combinelock_dummy");
	self.dummy:SetPos(self:GetPos() + (self:GetRight()*10) + (self:GetForward()*-7));
	self.dummy:SetAngles(self:GetAngles() + Angle(0, 180, 0));
	self.dummy:SetParent(self);
	self.dummy:Spawn();
	self.dummy:SetDoor(self.entity);
end;
...
-- A function to make it display red for a bit
function ENT:SetRed(dur)
	self:SetDTBool("SetRed", true);
	timer.Simple(dur, self.SetDTBool, self, "SetRed", false)
end;
