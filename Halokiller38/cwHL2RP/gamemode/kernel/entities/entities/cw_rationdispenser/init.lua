...
function ENT:SetRed(bool)
	self:SetDTBool("fail", bool);
end;
...
function ENT:ActivateRation(activator, duration, force)
	...
	if (force or !self.nextActivateRation or curTime >= self.nextActivateRation) then
		...
		Clockwork:CreateTimer("ration_"..self:EntIndex(), duration, 1, function()
										...
										if ( IsValid(self) and IsValid(entity) ) then
											...
											entity:CallOnRemove("CreateRation", function()
												if ( IsValid(activator) ) then
													local copyInstance = Clockwork.item:CreateInstance("ration");
													
													if (copyInstance) then
														copyInstance:SetData("Name", activator:Name().."'s Ration");
														local copyEntity = Clockwork.entity:CreateItem(
															activator, copyInstance, position, angles
														);
													end;
												end;
											end);
		...
		if (!self.nextUse or curTime >= self.nextUse) then
			if (activator:QueryCharacter("faction") == FACTION_CITIZEN) then
				if ( !self:IsLocked() and unixTime >= activator:GetCharacterData("nextration", 0) ) then
					...
					self:EmitSound("buttons/combine_button_locked.wav");
					self:SetRed(true);
					timer.Simple(3, self.SetRed, self, false);
				end;
			elseif ((!self.nextActivateRation or curTime >= self.nextActivateRation) and activator:QueryCharacter("faction") == FACTION_MPF) then
				self:Toggle();
			end;
