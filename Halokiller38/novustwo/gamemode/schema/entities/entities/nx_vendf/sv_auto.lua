...
function ENT:ToggleWithChecks(activator, player)
	...
	if (!self.nextUse or curTime >= self.nextUse) then
		if (player.cash(activator) > 8) then
			player.cash(activator, -22, "You have paid 22 caps for a drink from the vending machine.");
			...
			player.Notify(activator, "You do not have enough Caps!.");
		end;
	end;
end;
