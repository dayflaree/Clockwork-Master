...
NEXUS:HookDataStream("ManageCar", function(player, data)
...
function MOUNT:GetVehicleExit(player, vehicle)
	local available = {};
	local closest = {};
	...
	if ( closest[2] ) then
		nexus.player.SetSafePosition( player, closest[2] );
	end;
end;

-- A function to make a player exit a vehicle.
function MOUNT:MakeExitVehicle(player, vehicle)
...
function MOUNT:SpawnVehicle(player, itemTable)
	...
	if (currentlyUsed == 2) then
		nexus.player.Notify(player, "Two of these faction-based vehicles are already on the map!");

		return false;
	end;
	
	if ( table.HasValue(player.vehicles, itemTable) ) then
		nexus.player.Notify(player, "You have already spawned a "..itemTable.name..", go and look for it!");
		...
		if (player:GetPos():Distance(eyeTrace.HitPos) <= 512) then
			...
			if (!trace.HitSky) then
				nexus.player.Notify(player, "You can only spawn a vehicle outside!");
			...
			nexus.player.GiveProperty(player, vehicleEntity, true);

			player.vehicles[vehicleEntity] = itemTable;

			return vehicleEntity;
		else
			nexus.player.Notify(player, "You cannot spawn a vehicle that far away!");
...
function MOUNT:MakeVehicle(player, itemTable, position, angles)
