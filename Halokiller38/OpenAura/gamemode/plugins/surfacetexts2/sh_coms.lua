...
function COMMAND:OnRun(player, arguments)
	...
	openAura:StartDataStream( nil, "SurfaceTextAdd2", data );
...
openAura.command:Register(COMMAND, "TextAdd2");
...
function COMMAND:OnRun(player, arguments)
	...
	for k, v in pairs(PLUGIN.surfaceTexts) do
		if (v.position:Distance(position) <= 256) then
			openAura:StartDataStream( nil, "SurfaceTextRemove2", v.position );
...
openAura.command:Register(COMMAND, "TextRemove2");