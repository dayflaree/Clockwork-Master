...
function openAura.schema:OpenAuraInitialized()
	...
	for k, v in pairs( openAura.item:GetAll() ) do
		if ((v.category == "Junk" or v.Scrap) and !v.isBaseItem) then
...
function openAura.schema:Tick()
	...
	if (!self.nextTrashSpawns) then
		self.nextTrashSpawns = curTime + math.random(1600, 1800);
	end;
...
function openAura.schema:PlayerUseUnknownItemFunction(player, itemTable, itemFunction)
	if (openAura.player:HasFlags(player, "T") and itemFunction == "Bullets" and itemTable.cost) then
	...
	elseif (itemFunction == "Caps" and itemTable.category == "Junk" and openAura.player:HasFlags(player, "S")) then
		local useSounds = {"buttons/button5.wav", "buttons/button4.wav"};
		local cashBack = itemTable.worth or itemTable.cost;
		
		player:UpdateInventory(itemTable.uniqueID, -1, true);
		player:EmitSound( useSounds[ math.random(1, #useSounds) ] );
		
		openAura.player:GiveCash(player, cashBack, "scrapped some junk");
	end;
end;
...
function openAura.schema:GetPlayerDefaultInventory(player, character, inventory)
	if (character.faction == FACTION_CEDA) then
		inventory["aura_flashlight"] = 1;
		inventory["ranger_uniform"] = 1;
		inventory["kevlar"] = 1;
		inventory["backpack"] = 1;
		inventory["rcs_p228"] = 1;
		inventory["ammo_smg1"] = 4;
		inventory["weapon_ak74m"] = 1;
		inventory["ammo_pistol"] = 3;
		inventory["storage_jacket"] = 1;
		inventory["aura_batterycharger"] = 1;
		inventory["handheld_radio"] = 1;

	
		elseif (character.faction == FACTION_STALKER) then
		inventory["aura_flashlight"] = 1;
		inventory["stalker_uniform01"] = 1;
		inventory["backpack"] = 1;

	
		elseif (character.faction == FACTION_BANDIT) then
		inventory["aura_flashlight"] = 1;
		inventory["backpack"] = 1;
		inventory["bandit_uniform01"] = 1;

	
		elseif (character.faction == FACTION_NAZI) then
		inventory["aura_flashlight"] = 1;
		inventory["backpack"] = 1;
		inventory["rcs_p228"] = 1;
		inventory["ammo_smg1"] = 4;
		inventory["kevlar"] = 1;
		inventory["weapon_ak74u"] = 1;
		inventory["ammo_pistol"] = 3;
		inventory["storage_jacket"] = 1;
		inventory["handheld_radio"] = 1;

	
		elseif (character.faction == FACTION_COMMUNIST) then
		inventory["aura_flashlight"] = 1;
		inventory["backpack"] = 1;
		inventory["rcs_p228"] = 1;
		inventory["ammo_smg1"] = 4;
