...
ITEM.attachmentOffsetAngles = Angle(-90, -74.57, 0);
ITEM.attachmentOffsetVector = Vector(0, 0.66, 3.70);
ITEM.access = "U";
ITEM.cost = 7;
...
function ITEM:GetAttachmentVisible(player, entity)
	if ( player:GetSharedVar("hat1") ) then
		return true;
	end;
end;
...
function ITEM:GetLocalAmount(amount)
	if ( openAura.Client:GetSharedVar("hat1") ) then
		return amount - 1;
	else
		return amount;
	end;
...
function ITEM:HasPlayerEquipped(player, arguments)
	return player:GetSharedVar("hat1");
end;

-- Called when a player has unequipped the item.
function ITEM:OnPlayerUnequipped(player, arguments)
	local skullMaskGear = openAura.player:GetGear(player, "hat1");
	
	if ( player:GetSharedVar("hat1") and IsValid(skullMaskGear) ) then
		player:SetCharacterData("hat1", nil);
		player:SetSharedVar("hat1", false);
		
		if ( IsValid(hat1Gear) ) then
			hat1Gear:Remove();
		end;
	end;
...
function ITEM:OnDrop(player, position)
	if (player:GetSharedVar("hat1") and player:HasItem(self.uniqueID) == 1) then
...
function ITEM:OnUse(player, itemEntity)
	if ( player:Alive() and !player:IsRagdolled() ) then
		openAura.player:CreateGear(player, "hat1", self);
		
		player:SetCharacterData("hat1", true);
		player:SetSharedVar("hat1", true);
		player:UpdateInventory(self.uniqueID);
