...
function GM.TakeResource(ply,cmd,args)
	...
	if !(SPropProtection.PlayerIsPropOwner(ply, ent) or SPropProtection.IsBuddy(ply, ent)) then return end
...
function GM.MakeCampfire(ply,cmd,args)
	if GetConVarNumber("gms_Campfire") == 1 then
		...
		if (SPropProtection.PlayerIsPropOwner(ply, ent) or SPropProtection.IsBuddy(ply, ent)) then		
...
function PlayerMeta:PickupResourceEntity(ent)
	...
	if !(SPropProtection.PlayerIsPropOwner(self, ent) or SPropProtection.IsBuddy(self, ent)) then return end
...
function gms_addbuildsiteresource( ent_resourcedrop, ent_buildsite )
	...
	if ent_resourcedrop_owner != nil and ent_buildsite_owner != nil and ply != nil and ent_resourcedrop:IsPlayerHolding() then
		if SPropProtection.PlayerIsPropOwner(ply, ent_buildsite) or SPropProtection.IsBuddy(ply, ent_buildsite) then
