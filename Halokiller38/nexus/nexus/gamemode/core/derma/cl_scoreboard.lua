...
function PANEL:Init()
	...
	nexus.scoreboard = self;
	nexus.scoreboard:Rebuild();
end;
	...
	for k, v in ipairs( g_Player.GetAll() ) do
		if ( v:HasInitialized() ) then
			...
			if (class) then
				...
				if ( nexus.mount.Call("PlayerShouldShowOnScoreboard", v) ) then
	...
	if (table.Count(classes) > 0) then
		...
		for k, v in pairs(classes) do
			...
			for k2, v2 in pairs(v.players) do
				...
				self.currentFaction = nexus.player.GetFaction(v2);
				self.currentPlayer = v2;
				self.currentClass = g_Team.GetName( v2:Team() );
				self.currentModel = v2:GetModel();
				self.currentSkin = v2:GetSkin();
				self.currentName = v2:Name();
				
				panelList:AddItem( vgui.Create("nx_ScoreboardItem", self) ) ;
			end;
...
vgui.Register("nx_Scoreboard", PANEL, "DFrame");
...
function PANEL:Init()
	SCOREBOARD_PANEL = true;
		...
		local info = {
			doesRecognise = nexus.player.DoesRecognise(parent.currentPlayer),
		...
		info.text = nexus.mount.Call("GetPlayerScoreboardText", info.player);
		
		nexus.mount.Call("ScoreboardAdjustPlayerInfo", info);
		...
		if (info.doesRecognise) then
			...
			self.spawnIcon:SetImage("nexus/unknown");
		...
		function self.spawnIcon.DoClick(spawnIcon)
			local options = {};
				nexus.mount.Call("GetPlayerScoreboardOptions", info.player, options);
...
vgui.Register("nx_ScoreboardItem", PANEL, "DPanel");