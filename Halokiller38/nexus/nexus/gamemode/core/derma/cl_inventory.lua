...
function PANEL:Init()
	...
	function self.spawnIcon.DoClick(spawnIcon)
		...
		if (#itemFunctions > 0) then
			...
			nexus.mount.Call("PlayerAdjustItemMenu", self.item, menu, itemFunctions);
...
function PANEL:Init()
	...
	function self.spaceUsed.Paint(spaceUsed)
		...
		NEXUS:DrawRoundedGradient(0, 0, 0, spaceUsed:GetWide(), spaceUsed:GetTall(), color);
		NEXUS:DrawRoundedGradient( 0, 0, 0, width, spaceUsed:GetTall(), Color(139, 215, 113, 255) );
	end;
end;
