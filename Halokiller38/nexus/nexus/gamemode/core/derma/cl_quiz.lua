...
function PANEL:Populate()
	...
	self.questionsForm:SetName( nexus.quiz.GetName() );
	...
	for k, v in pairs(questions) do
		if (ScrW() < 1024) then
			...
			function panel:OnSelect(index, value, data)
				NEXUS:StartDataStream( "QuizAnswer", {key, index} );
			end;
	...
	self.continueButton:SetCallback(function(panel)
		NEXUS:StartDataStream("QuizCompleted", true);
	end);
...
vgui.Register("nx_Quiz", PANEL, "DPanel");