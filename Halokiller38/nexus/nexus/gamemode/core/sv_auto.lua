...
resource.AddFile("materials/nexus/nexus31.vtf");
resource.AddFile("materials/nexus/nexus31.vmt");
...
local gradientTexture = nexus.schema.GetOption("gradient");
...
function NEXUS:PlayerDisconnected(player)
	...
	if ( player:HasInitialized() ) then
		if (nexus.mount.Call("PlayerCharacterUnloaded", player) != true) then
...
function NEXUS:PlayerDropItem(player, itemTable, position, entity)
	if ( IsValid(entity) and nexus.item.IsWeapon(itemTable) ) then
		local secondaryAmmo = nexus.player.TakeSecondaryAmmo(player, itemTable.weaponClass);
		local primaryAmmo = nexus.player.TakePrimaryAmmo(player, itemTable.weaponClass);
...
function NEXUS:PlayerCanDropWeapon(player, itemTable, noMessage)
	if ( nexus.player.GetSpawnWeapon(player, itemTable.weaponClass) ) then
		if (!noMessage) then
...
function NEXUS:PhysgunDrop(player, entity)
	if ( !entity:IsPlayer() ) then
		if (entity.lastCollisionGroup) then
			nexus.entity.ReturnCollisionGroup(entity, entity.lastCollisionGroup);
		end;
	else
...
function NEXUS:EntityHandleMenuOption(player, entity, option, arguments)
	...
	if ( class == "nx_item" and (arguments == "nx_itemTake" or arguments == "nx_itemUse") ) then
		if ( nexus.entity.BelongsToAnotherCharacter(player, entity) ) then
			nexus.player.Notify(player, "You cannot pick up items you dropped on another character!");
	...
	elseif (class == "nx_item" and arguments == "nx_itemAmmo") then
		local itemTable = entity.item;
		
		if ( itemTable and nexus.item.IsWeapon(itemTable) ) then
	...
	elseif (class == "nx_cash" and arguments == "nx_cashTake") then
		if ( nexus.entity.BelongsToAnotherCharacter(player, entity) ) then
			nexus.player.Notify(player, "You cannot pick up cash you dropped on another character!");
