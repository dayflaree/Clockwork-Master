...
function SWEP:CSShootBullet( dmmg, recoil, amount, spread )
	if CLIENT then --in case if someone joined the server, and the font WASNT added.
		...
		if (!self.Weapon:GetNetworkedBool("Silenced") and !self.NoDSPEffects) then
			for k, v in ipairs(player.GetAll()) do
				if ( v:HasInitialized() ) then
					if (v:GetShootPos():Distance( self.Owner:GetShootPos() ) < 256) then
	...
	rcsbul1.Callback = function(attacker, bulletTrace, damageInfo)
		if ( !ValidEntity(bulletTrace.Entity) or (bulletTrace.Entity:GetClass() != "prop_vehicle_jeep"
...
function SWEP:Think()
	...
	if self.Owner:KeyDown(IN_FORWARD | IN_BACK | IN_MOVELEFT | IN_MOVERIGHT) and self.Owner:KeyDown(IN_SPEED) then
		...
		if(self.ShootafterTakeout <= CurTime() && deployDELAY <= CurTime())then
			if self.Owner:KeyPressed(IN_ATTACK2) || (self.Owner:KeyDown(IN_ATTACK2) && self.Weapon:GetNetworkedBool( "Ironsights")==false ) then --ironsights
				if ( nexus.player.GetWeaponRaised(self.Owner) ) then
