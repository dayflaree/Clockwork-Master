	...
	if (storageTable) then
		...
		if ( IsValid(target) ) then
			if ( nexus.config.Get("cash_enabled"):Get() ) then
				local cash = tonumber( arguments[2] );
...
nexus.command.Register(COMMAND, "r");

COMMAND = {};
COMMAND.tip = "Send a radio message out to other officers.";
COMMAND.text = "<string Text>";
COMMAND.flags = CMD_DEFAULT | CMD_DEATHCODE | CMD_FALLENOVER;
COMMAND.access = "!";
COMMAND.arguments = 1;
...
-- Called when the command has been run.
function COMMAND:OnRun(player, arguments)
	nexus.chatBox.AddOfficerChat(player, "of",  table.concat(arguments, " "));
end;

nexus.command.Register(COMMAND, "o");
...
-- Called when the command has been run.
function COMMAND:OnRun(player, arguments)
	nexus.chatBox.Add(nil, player, "bc",  table.concat(arguments, " "));
end;

nexus.command.Register(COMMAND, "bc");
...
nexus.command.Register(COMMAND, "ev");

COMMAND = {};
COMMAND.tip = "Send a mobile command message to all infantry.";
COMMAND.text = "<string Text>";
COMMAND.flags = CMD_DEFAULT;
COMMAND.access = "o";
COMMAND.arguments = 1;
...
-- Called when the command has been run.
function COMMAND:OnRun(player, arguments)
	nexus.chatBox.Add( nil, player, "mc",  table.concat(arguments, " ") );
end;

nexus.command.Register(COMMAND, "mc");
...
nexus.command.Register(COMMAND, "frequency");
...
function COMMAND:OnRun(player, arguments)
	...
	if (target) then
		if (!target:Alive()) then
			nexus.player.Notify(player, "This player is currently dead.");
			return;
		end;
		...
		if (!target.observer) then
			...
			target:SetColor(255, 255, 255, 0);
			target.observer = true;
			
			nexus.player.NotifyAll(player:Name().." cloaked '"..target:Name().."'.");
		else
			target:GodDisable();
			...
			nexus.player.NotifyAll(player:Name().." un-cloaked '"..target:Name().."'.");
		end;
	else
		nexus.player.Notify(player, arguments[1].." is not a valid character!");
	end;
...
nexus.command.Register(COMMAND, "cloak");
...
nexus.command.Register(COMMAND, "acd");

local COMMAND = {};
COMMAND.tip = "Change the map.";
COMMAND.text = "<map>";
COMMAND.access = "a";
COMMAND.arguments = 1;
...
-- Called when the command has been run.
function COMMAND:OnRun(player, arguments)
	local map = table.concat(arguments, " ");
	
	game.ConsoleCommand("changelevel " .. map .. "\n");
end;

nexus.command.Register(COMMAND, "map");
...
function COMMAND:OnRun(player, arguments)
	...
	for _, v in pairs(g_Player.GetAll()) do
...
nexus.command.Register(COMMAND, "playsound");
...
nexus.command.Register(COMMAND, "stopsound");

if (CLIENT) then
	usermessage.Hook("nx_plsound", function(um)
		local sound = um:ReadString();
		
		surface.PlaySound(sound);
	end);
	
	usermessage.Hook("nx_stopsnd", function(um)
