...
function PANEL:Init()
	...
	self.subLabel:SetPos( self.titleLabel.x + (self.titleLabel:GetWide() / 2) - (self.subLabel:GetWide() / 2), self.titleLabel.y + self.titleLabel:GetTall() - 4 );
...
function PANEL:FadeInModelPanel(model)
	...
	if (panel) then
		x, y = panel.x + panel:GetWide() - 16, panel.y - 32;
	end;
...
function PANEL:OpenPanel(stage, data, Callback)
	...
	if (panel) then
		panel:FadeOut(1, function()
			...
			nexus.character.activePanel:SetPos( ScrW() * 0.3, ScrH() * 0.25 );
...
function PANEL:SetModel(model)
	...
	self.modelPanel.Entity:ResetSequence( self.modelPanel.Entity:SelectWeightedSequence(1) );
end;
