...
function PANEL:Init()
	...
	self.label:SetText("");
	self.label:SetTextColor( nexus.schema.GetColor("white") );
...
function PANEL:Paint()
	if ( self:GetPaintBackground() ) then
		...
		if ( self:IsButton() and self:IsHovered() ) then
			NEXUS:DrawRoundedGradient( 4, x, y, width, height, Color(255, 255, 255, 50) );
		end;
	end	
...
vgui.Register("nx_InfoText", PANEL, "DPanel");