...
function NEXUS:PlayerShouldShowOnScoreboard(player)
	if(player:GetNetworkedBool("cloak")) then
		return false
	else
		return true
	end
...
function NEXUS:CalcView(player, origin, angles, fov)
	...
	if ( IsValid(weapon) ) then
		...
		if (!weaponRaised) then
			...
			elseif (weapon.RunArmAngle) then
				anglesMod = weapon.RunArmAngle;
			elseif (weapon.LoweredAngles) then
				anglesMod = weapon.LoweredAngles;
			elseif (weapon.RunAng) then
				anglesMod.x = weapon.RunAng.p;
				anglesMod.y = weapon.RunAng.y;
				anglesMod.z = weapon.RunAng.r;
			end;
			
			if (itemTable and itemTable.loweredOrigin) then
				originMod = itemTable.loweredOrigin;
			elseif (weapon.RunArmOffset) then
				originMod = weapon.RunArmOffset;
			elseif (weapon.LoweredPos) then
				originMod = weapon.LoweredPos;
			elseif (weapon.RunPos) then
				originMod = weapon.RunPos;
			end;
			
			originalAngles:RotateAroundAxis(originalAngles:Right(), anglesMod.x);
			originalAngles:RotateAroundAxis(originalAngles:Up(), anglesMod.y);
			originalAngles:RotateAroundAxis(originalAngles:Forward(), anglesMod.z);
...
function NEXUS:GetAdminESPInfo(info)
	for k, v in ipairs( g_Player.GetAll() ) do
		if ( v:HasInitialized() ) then
			...
			if(v:GetNetworkedBool("cloak")) then
					text = ""
			end
		end;
	end;
