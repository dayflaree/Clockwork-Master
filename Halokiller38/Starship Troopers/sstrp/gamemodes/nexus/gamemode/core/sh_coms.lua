				...
				if (steamName) then
					if (duration > 0) then
						...
						if (hours >= 1) then
							...
							nexus.player.NotifyAll(player:Name().." has banned '"..steamName.."' for "..math.Round(duration / 60).." minute(s) ("..reason..").");
						end;
					else
...
-- Called when the command has been run.
function COMMAND:OnRun(player, arguments)
	player:PrintMessage(2, "######## Nexus -> Config ########");
...
function COMMAND:OnRun(player, arguments)
	...
	if ( IsValid(weapon) ) then
		...
		if (itemTable) then
			if ( nexus.mount.Call("PlayerCanDropWeapon", player, itemTable) ) then
				local trace = player:GetEyeTraceNoCursor();
