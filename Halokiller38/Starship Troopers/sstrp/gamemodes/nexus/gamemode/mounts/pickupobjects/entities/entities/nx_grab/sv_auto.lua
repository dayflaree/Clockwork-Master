...
function ENT:Initialize()
	...
	self.player = NULL;
	self.mount = nexus.mount.Get("Pickup Objects");
...
-- Called each frame.
function ENT:Think()
	if (IsValid(self.player) and IsValid(self.target) and self.mount) then
		if ( !self.mount:CalculatePosition(self.player) ) then
			self.mount:ForceDropEntity(self.player);
		end;
	else
		self:Remove();
	end;
