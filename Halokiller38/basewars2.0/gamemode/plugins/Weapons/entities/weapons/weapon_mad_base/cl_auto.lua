...
function SWEP:DrawHUD()
	...
	if (self.Weapon:GetDTBool(1)) then
		self.distMul = math.Approach(self.distMul, 0.75, FrameTime() * 10);
		self.crosshairSize = math.Approach(self.crosshairSize, 5, FrameTime() * 10);
	else
		self.distMul = math.Approach(self.distMul, 1.5, FrameTime() * 10);
		self.crosshairSize = math.Approach(self.crosshairSize, 6, FrameTime() * 10);
	end;
	
	local origin = LocalPlayer():GetEyeTraceNoCursor().HitPos:ToScreen();
	local distance = ((self.Primary.Cone * 275) + (((self.Primary.Cone * 275) * (ScrH() / 720))) * (1 / self:CrosshairAccuracy())) * self.distMul;
	...
	for i = self.crosshairAngle / 2, 360 - self.crosshairAngle / 2, self.crosshairAngle do
		...
		local offset = self.crosshairSize / 2;
		
		surface.SetDrawColor(0, 0, 0, 255);
		surface.DrawRect(origin.x + addX - offset, origin.y + addY - offset, self.crosshairSize, self.crosshairSize);
		surface.SetDrawColor(255, 255, 255, 255);
		surface.DrawRect(origin.x + addX - offset + 1, origin.y + addY - offset + 1, self.crosshairSize - 2, self.crosshairSize - 2);
	end;
end;
