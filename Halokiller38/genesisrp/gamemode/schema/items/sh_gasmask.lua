...
function ITEM:GetAttachmentVisible(player, entity)
	if ( player:GetSharedVar("gasmask100") ) then
		return true;
	end;
end;
...
function ITEM:GetLocalAmount(amount)
	if ( openAura.Client:GetSharedVar("gasmask100") ) then
		return amount - 1;
	else
		return amount;
	end;
...
function ITEM:HasPlayerEquipped(player, arguments)
	return player:GetSharedVar("gasmask100");
end;

-- Called when a player has unequipped the item.
function ITEM:OnPlayerUnequipped(player, arguments)
	local skullMaskGear = openAura.player:GetGear(player, "gasmask100");
	
	if ( player:GetSharedVar("gasmask100") and IsValid(skullMaskGear) ) then
		player:SetCharacterData("gasmask100", nil);
		player:SetSharedVar("gasmask100", false);
		
		if ( IsValid(gasmask100Gear) ) then
			gasmask100Gear:Remove();
		end;
	end;
...
function ITEM:OnDrop(player, position)
	if (player:GetSharedVar("gasmask100") and player:HasItem(self.uniqueID) == 1) then
...
function ITEM:OnUse(player, itemEntity)
	if ( player:Alive() and !player:IsRagdolled() ) then
		openAura.player:CreateGear(player, "gasmask100", self);
		
		player:SetCharacterData("gasmask100", true);
		player:SetSharedVar("gasmask100", true);
		player:UpdateInventory(self.uniqueID);
