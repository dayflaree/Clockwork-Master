...
--[[
ToDo: Ported from our old Omicron gamemode... FIX UP.
--]]
...
if (SERVER) then
	...
	function PLUGIN:DrawWeaponInformation(weapon, x, y, alpha)
		...
		if (primaryAmmo or secondaryAmmo or weapon.Instructions) then
	...
	function PLUGIN:HUDPaint()
		...
		if (IsValid(activeWeapon) and self.displayAlpha > 0) then
			...
			if (IsValid(currentWeapon)) then
				...
				self:DrawWeaponInformation(currentWeapon, weaponInfoX, weaponInfoY, self.displayAlpha);
	...
	function PLUGIN:BindPress(ply, bind, press)
		local activeWeapon = LocalPlayer():GetActiveWeapon();
		local newWeapons = {};
		local curTime = UnPredictedCurTime();
		local weapons = LocalPlayer():GetWeapons();
