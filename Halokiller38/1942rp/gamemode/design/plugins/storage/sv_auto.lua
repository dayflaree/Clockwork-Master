...
BLUEPRINT:HookDataStream("ContainerPassword", function(player, data)
		...
		if ( blueprint.entity.IsPhysicsEntity(entity) ) then
			...
			if ( PLUGIN.containers[model] ) then
				...
				if (entity.password == password) then
					...
					blueprint.player.Notify(player, "You have entered an incorrect password!");
				end;
			end;
		end;
	end;
...
function PLUGIN:GetRandomItem(class)
	...
	if (#self.randomItems > 0) then
		...
		if (item) then
			local itemTable = blueprint.item.Get( item[1] );
...
function PLUGIN:SaveStorage()
	...
	BLUEPRINT:SaveDesignData("plugins/storage/"..game.GetMap(), storage);
end;
...
function PLUGIN:LoadStorage()
	...
	local storage = BLUEPRINT:RestoreDesignData( "plugins/storage/"..game.GetMap() );
	
	for k, v in pairs(storage) do
		for k2, v2 in pairs(v.inventory) do
			local itemTable = blueprint.item.Get(k2);
...
function PLUGIN:OpenContainer(player, entity, weight)
	...
	if (entity.message) then
		umsg.Start("bp_StorageMessage", player);
			umsg.Entity(entity);
