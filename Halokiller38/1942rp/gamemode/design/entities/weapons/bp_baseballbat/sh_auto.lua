...
if (SERVER) then
	...
	for k, v in pairs( g_File.Find("../models/weapons/v_basball.*") ) do
		resource.AddFile("models/weapons/"..v);
	end;
	
	for k, v in pairs( g_File.Find("../models/weapons/w_basball.*") ) do
		resource.AddFile("models/weapons/"..v);
	end;
end;
	...
	if (SERVER) then
		...
		if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 64) then
			if ( IsValid(trace.Entity) ) then
				local player = blueprint.entity.GetPlayer(trace.Entity);
				local strength = blueprint.attributes.Fraction(self.Owner, ATB_STRENGTH, 10, 5);
				...
				if ( trace.Entity:IsPlayer() or trace.Entity:IsNPC() ) then
					...
					timer.Simple(FrameTime() * 0.5, function()
						if ( IsValid(trace.Entity) ) then
							trace.Entity:TakeDamageInfo( BLUEPRINT:FakeDamageInfo(self.Primary.Damage + strength, self, self, trace.HitPos, DMG_CLUB, 2) );
						end;
					end);
					...
					if ( IsValid(physicsObject) ) then
						...
						if (!player) then
							timer.Simple(FrameTime() * 0.5, function()
								if ( IsValid(trace.Entity) ) then
									trace.Entity:TakeDamageInfo( BLUEPRINT:FakeDamageInfo( (self.Primary.Damage * 2) + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2 ) );
								end;
							end);
							...
							timer.Simple(FrameTime() * 0.5, function()
								if ( IsValid(trace.Entity) ) then
									trace.Entity:TakeDamageInfo( BLUEPRINT:FakeDamageInfo(self.Primary.Damage + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2) );
								end;
							end);
