...
ITEM.name = "Heavy S.T.A.L.K.E.R Base";
ITEM.model = "models/stalkertnb/outfits/exo_loner.mdl";
ITEM.batch = 1;
ITEM.weight = 7.0;
ITEM.useText = "Wear";
ITEM.business = true;
...
ITEM.description = "The Whole Heavy Elite S.T.A.L.K.E.R Suit.";
...
function ITEM:OnUse(player, itemEntity)
	if ( player:Alive() and !player:IsRagdolled() ) then
		if (openAura.player:GetFaction(player) == FACTION_STALKER
		or openAura.player:GetFaction(player) == FACTION_DUTY
		or openAura.player:GetFaction(player) == FACTION_FREEDOM
		or openAura.player:GetFaction(player) == FACTION_BANDIT
		or openAura.player:GetFaction(player) == FACTION_MILITARY
		or openAura.player:GetFaction(player) == FACTION_MERC
		or openAura.player:GetFaction(player) == FACTION_MONOLITH) then
			...
			openAura.player:Notify(player, "You cannot wear these clothes as you are not a wastelander!");
		end;
	else
