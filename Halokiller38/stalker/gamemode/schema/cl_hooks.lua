...
function openAura.schema:MenuItemsDestroy(menuItems)
	if ( !openAura.player:HasFlags(openAura.Client, "T") ) then
		menuItems:Destroy( openAura.option:GetKey("name_buisness") );
	end;
end;
	...
	/*if (itemTable.OnUse) then
		...
		if (itemTable.weaponLevel and itemTable.nextWeaponID) then
			if (itemTable.weaponLevel != 10) then
				...
				SetToolTip("This upgrade will cost you "..FORMAT_CASH(itemTable.weaponLevel * 3000, true)..".");
					end;
				end;
			end;
		end;
...
/*function openAura.schema:PlayerAdjustItemFunctions(itemTable, itemFunctions)
		if (itemTable.cost) then
			itemFunctions[#itemFunctions + 1] = "RU";
		end;
	end;
*/
...
function openAura.schema:GetScreenTextInfo()
	...
	if ( openAura.Client:GetSharedVar("beingTied") ) then
		return {
