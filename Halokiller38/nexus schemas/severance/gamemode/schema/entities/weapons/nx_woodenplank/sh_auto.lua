...
SWEP.Purpose = "What could be better than smacking someone with a plank? Smacking someone with a plank that has nails in it.";
SWEP.Contact = "";
	...
	if (SERVER) then
		...
		if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 64) then
			if ( IsValid(trace.Entity) ) then
				...
				local strength = nexus.attributes.Fraction(self.Owner, ATB_STRENGTH, 10, 5);
				...
				if ( trace.Entity:IsPlayer() or trace.Entity:IsNPC() ) then
					...
					timer.Simple(FrameTime() * 0.5, function()
						if ( IsValid(trace.Entity) ) then
							trace.Entity:TakeDamageInfo( NEXUS:FakeDamageInfo(self.Primary.Damage + strength, self, self, trace.HitPos, DMG_CLUB, 2) );
						end;
					end);
					...
					if ( IsValid(physicsObject) ) then
						...
						if (!player) then
							timer.Simple(FrameTime() * 0.5, function()
								if ( IsValid(trace.Entity) ) then
									trace.Entity:TakeDamageInfo( NEXUS:FakeDamageInfo( (self.Primary.Damage * 2) + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2 ) );
								end;
							end);
							...
							timer.Simple(FrameTime() * 0.5, function()
								if ( IsValid(trace.Entity) ) then
									trace.Entity:TakeDamageInfo( NEXUS:FakeDamageInfo(self.Primary.Damage + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2) );
								end;
							end);
