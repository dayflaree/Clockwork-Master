...
function ENT:SpawnFunction(player, trace)
	...
	local entity = ents.Create("nx_basezombie");
...
function ENT:DoDeath()
	if (!self.IsDead) then
		...
		self.DamageForce = NEXUS:ConvertForce(self.DamageForce * 192);
		
		local ragdoll = nexus.entity.MakeIntoRagdoll( self, nil, (self:GetVelocity() * 1.5) + self.DamageForce, self:GetAngles() - Angle(0, -45, 0) );
		
		if ( IsValid(ragdoll) ) then
			ragdoll:SetCollisionGroup(COLLISION_GROUP_WEAPON);
			
			nexus.entity.Decay(ragdoll, 120);
...
function ENT:OnTakeDamage(damageInfo)
	...
	NEXUS:CreateBloodEffects(damagePosition, 1, self, damageForce);
end;
...
function ENT:TaskStart_FindEnemy(data)
	...
	for k, v in ipairs( ents.FindInSphere(position, 1024) ) do
		if ( v:GetClass() == "nx_flare" and ( self:Visible(v) or v:Visible(self) ) ) then
