...
function MOUNT:NexusInitPostEntity()
	if ( nexus.config.Get("remove_map_physics"):Get() ) then
		for k, v in ipairs( ents.FindByClass("prop_vehicle*") ) do
			v:Remove();
		end;
	end;
end;
...
function MOUNT:InitPostEntity()
	...
	if ( nexus.config.Get("remove_map_physics"):Get() ) then
		for k, v in ipairs( NEXUS:GetPhysicsEntities() ) do
			v:Remove();
		end;
	end;
	...
	if (string.lower( game.GetMap() ) == "md_venetianredux_b2") then
		...
		for k, v in ipairs( ents.FindByClass("point_servercommand") ) do
			v:Remove();
		end;
		
		for k, v in ipairs( ents.FindByClass("lua_run") ) do
			v:Remove();
		end;
	end;
	
	for k, v in ipairs( ents.FindByClass("func_button") ) do
		for k2, v2 in ipairs(gunButtons) do
