...
-- A function to get the mount directory.
function NEXUS:GetMountDirectory()
...
function NEXUS:HandlePlayerJumping(player)
	...
	if (player.m_bJumping) then
		...
		if (player.m_bJumping) then
			player.CalcIdeal = nexus.animation.GetForModel(player:GetModel(), "jump");
...
function NEXUS:HandlePlayerDucking(player, velocity)
	if ( player:Crouching() ) then
		...
		player.CalcIdeal = nexus.animation.GetForModel(model, animationAct);
...
function NEXUS:HandlePlayerDriving(player)
	if ( player:InVehicle() ) then
		player.CalcIdeal = nexus.animation.GetForModel(player:GetModel(), "sit");
...
function NEXUS:CalcMainActivity(player, velocity)
	...
	player.CalcIdeal = nexus.animation.GetForModel(model, animationAct.."_idle");
	...
	and !self:HandlePlayerSwimming(player) ) then
		...
		if ( player:IsRunning() or player:IsJogging() ) then
			player.CalcIdeal = nexus.animation.GetForModel(model, animationAct.."_run");
		elseif (velLength > 0.5) then
			player.CalcIdeal = nexus.animation.GetForModel(model, animationAct.."_walk");
		end;
	end;
...
function NEXUS:DoAnimationEvent(player, event, data)
	...
	if (event == PLAYERANIMEVENT_ATTACK_PRIMARY) then
		local gestureSequence = nexus.animation.GetForModel(model, animationAct.."_attack");
	...
	elseif (event == PLAYERANIMEVENT_RELOAD) then
		local gestureSequence = nexus.animation.GetForModel(model, animationAct.."_reload");
...
if (SERVER) then
				...
				menu:AddOption(option, function()
					if (type(arguments) == "table" and arguments.arguments) then
						if (!arguments.Callback) then
							nexus.entity.ForceMenuOption(entity, option, arguments.uniqueID);
						else
	...
	function NEXUS:GetGradientTexture()
	...
	function NEXUS:DrawPlayerInfo()
		if ( nexus.mount.Call("PlayerCanSeePlayerInfo") ) then
			...
			if (#information > 0 or #subInformation > 0) then
				...
				local y = scrH - height - (scrH * 0.025);
				local x = scrW * 0.025;
	...
	for k, v in pairs(self.Timers) do
		if (!v.paused) then
			if (curTime >= v.nextCall) then
				...
				if (!success) then
					ErrorNoHalt("Nexus -> the "..tostring(k).." timer has failed to run.");
					ErrorNoHalt(value);
				end;
