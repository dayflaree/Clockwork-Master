...
function nexus.player.GetAllProperty()
	for k, v in pairs(nexus.player.property) do
		if ( !IsValid(v) ) then
			nexus.player.property[k] = nil;
		end;
	end;
...
function nexus.player.DisableProperty(player)
	...
	for k, v in pairs( nexus.player.GetAllProperty() ) do
		if ( IsValid(v) and uniqueID == nexus.entity.QueryProperty(v, "uniqueID") ) then
			...
			v:SetOwnerKey(nil);
			v:SetNetworkedBool("sh_Owned", false);
...
function nexus.player.GiveProperty(player, entity, networked, removeDelay)
	...
	nexus.player.property[ entity:EntIndex() ] = entity;
	...
	if (key and uniqueID) then
		...
		if (nexus.entity.QueryProperty(entity, "uniqueID") == uniqueID) then
			if (nexus.entity.QueryProperty(entity, "key") == key) then
				...
				nexus.player.property[ entity:EntIndex() ] = nil;
...
function nexus.player.GetGeneratorCount(player)
	local generators = nexus.generator.GetAll();
	local count = 0;
	
	for k, v in pairs(generators) do
		count = count + nexus.player.GetPropertyCount(player, k);
	end;
...
function nexus.player.GetPropertyEntities(player, class)
	local uniqueID = player:UniqueID();
	local entities = {};
	local key = player:QueryCharacter("key");
...
function nexus.player.DropWeapons(player)
	...
	if ( player:IsRagdolled() ) then
		...
		for k, v in pairs(ragdollWeapons) do
			if (v.canHolster) then
				...
				if (itemTable) then
					if ( nexus.mount.Call("PlayerCanDropWeapon", player, itemTable, true) ) then
						local info = {
							itemTable = itemTable,
