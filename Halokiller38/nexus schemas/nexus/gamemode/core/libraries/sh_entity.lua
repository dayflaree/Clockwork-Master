...
if (SERVER) then
	...
	function nexus.entity.StatueRagdoll(entity, forceLimit)
	...
	function nexus.entity.ReturnCollisionGroup(entity, collisionGroup)
		if ( IsValid(entity) ) then
			...
			if ( IsValid(physicsObject) ) then
				if ( !physicsObject:IsPenetrating() ) then
					...
					NEXUS:CreateTimer("Collision Group: "..index, 1, 1, function()
						nexus.entity.ReturnCollisionGroup(entity, collisionGroup);
					end);
				end;
			end;
		end;
	...
	function nexus.entity.BelongsToAnotherCharacter(player, entity)
		local uniqueID = nexus.entity.QueryProperty(entity, "uniqueID");
		local key = nexus.entity.QueryProperty(entity, "key");
