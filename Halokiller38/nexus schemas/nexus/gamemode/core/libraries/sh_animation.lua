...
function nexus.animation.AddModel(class, model)
	local lowerModel = string.lower(model);
		nexus.animation.models[lowerModel] = class;
	return lowerModel;
end;

-- A function to add an override.
function nexus.animation.AddOverride(model, key, value)
	local lowerModel = string.lower(model);
	
	if ( !nexus.animation.override[lowerModel] ) then
		nexus.animation.override[lowerModel] = {};
	end;
	
	nexus.animation.override[lowerModel][key] = value;
end;

-- A function to get an animation for a model.
function nexus.animation.GetForModel(model, key)
	local lowerModel = string.lower(model);
	local animTable = nexus.animation.GetTable(lowerModel);
	local overrideTable = nexus.animation.override[lowerModel];
	
	if ( overrideTable and overrideTable[key] ) then
		return overrideTable[key];
	else
