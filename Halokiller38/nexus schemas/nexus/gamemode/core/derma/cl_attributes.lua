...
function PANEL:Rebuild()
	...
	if (#categories > 0 or #miscellaneous > 0) then
		local attributeName = string.lower( nexus.schema.GetOption("name_attribute") );
...
function PANEL:Init()
	...
	function self.baseBar.Paint(baseBar)
		...
		if (boostData.negative) then
			if (attributes[uniqueID] - boostData.boost < 0) then
				NEXUS:DrawRoundedGradient(0, 0, 0, boostData.width, baseBar:GetTall(), hinderColor);
			else
				boostData.width = math.min(boostData.width, width);
				
				NEXUS:DrawRoundedGradient(0, 0, 0, math.max(width - boostData.width, 0), baseBar:GetTall(), mainColor);
				NEXUS:DrawRoundedGradient(0, math.max(width - boostData.width, 0), 0, boostData.width, baseBar:GetTall(), hinderColor);
			end;
		else
			NEXUS:DrawRoundedGradient(0, 0, 0, width, baseBar:GetTall(), mainColor);
			NEXUS:DrawRoundedGradient(0, width, 0, math.min( boostData.width, baseBar:GetWide() ), baseBar:GetTall(), boostColor);
		end;
	end;
	...
	function self.progressBar.Paint(progressBar)
		...
		NEXUS:DrawRoundedGradient(0, 0, 0, progressBar:GetWide(), progressBar:GetTall(), color);
		NEXUS:DrawRoundedGradient(0, 0, 0, width, progressBar:GetTall(), progressColor);
	end;
end;
