...
function PANEL:Init()
	...
	self.characterMenuLabel:SetPos(scrW * 0.025, scrH * 0.025);
	...
	self.closeMenuLabel:SetPos(scrW - self.closeMenuLabel:GetWide() - (scrW * 0.025), scrH * 0.025);
...
function PANEL:Rebuild()
	...
	local originalY = (ScrH() * 0.05) + self.characterMenuLabel:GetTall();
	local width = 0;
	local y = originalY;
	local x = ScrW() * 0.025;
	...
	nexus.menu.navInfo.height = y - originalY;
	nexus.menu.navInfo.width = width;
	nexus.menu.navInfo.y = originalY;
	nexus.menu.navInfo.x = x;
end;
