...
surface.CreateFont("Arial", ScaleToWideScreen(72), 700, true, false, "nx_MenuTextBig");
...
function NEXUS:GetCustomBusinessInfoLabel(panel, itemTable) end;
	...
	elseif ( nexus.config.Get("enable_headbob"):Get() ) then
		if ( player:IsOnGround() ) then
			...
			if ( !nexus.player.IsNoClipping(player) ) then
				...
				local curTime = UnPredictedCurTime();
				local info = {speed = 1, yaw = 0.1, roll = 1};
...
function NEXUS:OnEntityCreated(entity)
...
function NEXUS:NexusItemEntityThink(itemTable, entity) end;

-- Called when an item entity is drawn.
function NEXUS:NexusItemEntityDraw(itemTable, entity) end;

-- Called when a cash entity is drawn.
function NEXUS:NexusCashEntityDraw(entity) end;

-- Called when a generator entity is drawn.
function NEXUS:NexusGeneratorEntityDraw(entity) end;

-- Called when a shipment entity is drawn.
function NEXUS:NexusShipmentEntityDraw(entity) end;
						...
						if ( nexus.mount.Call("ShouldDrawPlayerTargetID", entity) ) then
							if ( !nexus.player.IsNoClipping(entity) ) then
								if (g_LocalPlayer:GetShootPos():Distance(trace.HitPos) <= fadeDistance) then
									...
									local toScreen = ( trace.HitPos + Vector(0, 0, 4) ):ToScreen();
...
function NEXUS:PlayerAdjustMenuFunctions(itemTable, menuPanel, itemFunctions) end;
...
function NEXUS:HUDDrawScoreBoard()
	...
	if ( !g_LocalPlayer:HasInitialized() ) then
		...
		elseif ( nexus.mount.Call("ShouldDrawCharacterBackground") ) then
	...
	if ( NEXUS:GetSharedVar("sh_NoMySQL") ) then
		self:DrawRoundedGradient( 0, 0, 0, scrW, scrH, Color(0, 0, 0, 255) );
		draw.SimpleText("There server encountered an error connecting to MySQL.", introTextSmallFont, scrW / 2, scrH / 2, Color(179, 46, 49, 255), 1, 1);
		draw.SimpleText("The server owner should edit garrysmod/data/nexus/mysql.cfg.", introTextSmallFont, scrW / 2, (scrH / 2) + draw.GetFontHeight(introTextSmallFont) + 8, Color(179, 46, 49, 255), 1, 1);
	end;
...
function NEXUS:PostDrawBackgroundBlurs()
	...
	local position = nexus.mount.Call("GetChatBoxPosition");
