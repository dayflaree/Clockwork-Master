...
function ENT:OnRemove()
	if ( !RESISTANCE:IsShuttingDown() ) then
		-- local i;
		...
		if (self.inventory) then
			for k, v in pairs(self.inventory) do
				if (v > 0) then
					for i = 1, v do
						local item = resistance.entity.CreateItem( nil, k, self:GetPos() + Vector( 0, 0, math.random(1, 48) ), self:GetAngles() );
						
						if ( IsValid(item) ) then
							resistance.entity.CopyOwner(self, item);
						end;
					end;
				end;
			end;
		...
		if (self.cash and self.cash > 0) then
			resistance.entity.CreateCash( nil, self.cash, self:GetPos() + Vector( 0, 0, math.random(1, 48) ) );
		end;
