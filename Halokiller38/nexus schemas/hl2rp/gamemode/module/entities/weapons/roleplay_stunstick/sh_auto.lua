...
SWEP.Contact = "";
SWEP.Author	= "Dazzle Modifications";
...
function SWEP:DoHitEffects()
	...
	if ( ( (trace.Hit or trace.HitWorld) and self.Owner:GetPos():Distance(trace.HitPos) <= 96 ) ) then
		...
		elseif ( IsValid(trace.Entity) and resistance.entity.GetPlayer(trace.Entity) ) then
			self:SendWeaponAnim(ACT_VM_MISSCENTER);
			self:EmitSound("weapons/stunstick/stunstick_fleshhit"..math.random(1, 2)..".wav");
		else
			self:SendWeaponAnim(ACT_VM_MISSCENTER);
...
function SWEP:OnLowered()
	...
	if (SERVER) then
		...
		if (resistance.animation.GetModelClass( self.Owner:GetModel() ) == "civilProtection") then
...
function SWEP:DrawWorldModel()
	...
	if ( resistance.player.GetWeaponRaised(self.Owner) ) then
...
function SWEP:PrimaryAttack()
	...
	if (SERVER) then
		...
		if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 96) then
			if ( IsValid(trace.Entity) ) then
				local player = resistance.entity.GetPlayer(trace.Entity);
				local strength = resistance.attributes.Fraction(self.Owner, ATB_STRENGTH, 3, 1.5);
				...
				if ( trace.Entity:IsPlayer() ) then
					...
					if (trace.Entity:Health() > 10) then
						trace.Entity:TakeDamageInfo( RESISTANCE:FakeDamageInfo(1 + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2) );
					end;
					
					resistance.plugin.Call("PlayerStunEntity", self.Owner, trace.Entity);
				elseif ( IsValid( trace.Entity:GetPhysicsObject() ) ) then
					...
					if (!player or player:Health() > 10) then
						if (!player) then
							trace.Entity:TakeDamageInfo( RESISTANCE:FakeDamageInfo(5 + (strength * 2), self, self.Owner, trace.HitPos, DMG_CLUB, 2) );
						else
							trace.Entity:TakeDamageInfo( RESISTANCE:FakeDamageInfo(1 + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2) );
						end;
					end;
...
function SWEP:SecondaryAttack()
	if (SERVER) then
		...
		if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 96) then
			if ( IsValid(trace.Entity) ) then
				if ( self.Owner:GetShootPos():Distance(trace.HitPos) <= 64 and resistance.entity.IsDoor(trace.Entity) ) then
					self:SetNextPrimaryFire(CurTime() + 0.25);
					self:SetNextSecondaryFire(CurTime() + 0.25);
					
					if ( resistance.plugin.Call("PlayerCanKnockOnDoor", self.Owner, trace.Entity) ) then
						self:PlayKnockSound();
						
						resistance.plugin.Call("PlayerKnockOnDoor", self.Owner, trace.Entity);
					end;
				else
					self:PlayPushSound();
