...
function PANEL:Think()
	...
	if (!IsValid(self.entity) or self.entity:GetPos():Distance( g_LocalPlayer:GetPos() ) > 192) then
		self:Close(); self:Remove();
...
function PANEL:Populate()
	...
	function button.DoClick(button)
		...
		if ( IsValid(self.entity) ) then
			RESISTANCE:StartDataStream( "EditPaper", { self.entity, string.sub(textEntry:GetValue(), 0, 500) } );
		end;
	end;
...
vgui.Register("roleplay_EditPaper", PANEL, "DFrame");