...
function SCHEMA:MenuItemsDestroy(menuItems)
	if ( !nexus.player.HasFlags(g_LocalPlayer, "T") ) then
	...
	if ( IsValid(g_LocalPlayer) ) then
		if ( NEXUS:IsCharacterScreenOpen(true) ) then
			if (!self.musicSound) then
				self.musicSound = CreateSound(g_LocalPlayer, "music/hl2_song26.mp3");
...
function SCHEMA:PlayerAdjustColorModify(colorModify)
	...
	if ( choosingCharacter or nexus.menu.GetOpen() ) then
		...
		self.colorModify.brightness = math.Approach(self.colorModify.brightness, -0.1, interval);
		self.colorModify.contrast = math.Approach(self.colorModify.contrast, 1.5, interval);
		...
		self.colorModify.color = math.Approach(self.colorModify.color, color, interval);
	end;
...
function SCHEMA:GetScreenTextInfo()
	...
	elseif ( g_LocalPlayer:GetSharedVar("sh_TempKilled") ) then
		return {
			alpha = blackFadeAlpha,
			title = "THIS CHARACTER HAS BEEN KILLED FOR ONE HOUR",
			text = "You can still switch or create new characters."
		};
