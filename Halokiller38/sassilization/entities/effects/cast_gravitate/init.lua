...
function EFFECT:Init( data )
	...
	self.r,self.g,self.b,self.a = self.WeaponEnt:GetColor()
	
	self.StartAlpha = 200				--starting alpha of the particles
...
function EFFECT:Think( )
	...
	if LifeSpan > 0 then 
		...
		local particle = self.Emitter:Add( "particles/fire_glow",  startpos ) -- Defines the sprite used
			...
			particle:SetVelocity(Vector(0,0,math.Rand(165,365)))	--should be a bit faster than the other one... also...
			...
			particle:SetColor(self.r*tempColor,self.g*tempColor,self.b*tempColor)					--Particle colour, always has a blue tint
