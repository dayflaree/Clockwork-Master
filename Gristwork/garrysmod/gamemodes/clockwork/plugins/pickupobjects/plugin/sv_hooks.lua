...
function cwPickupObjects:PlayerThink(player, curTime, infoTable)
	if (IsValid(player.cwHoldingEnt)) then
		local entityPosition = player.cwHoldingEnt:GetPos();
		local playerShootPosition = player:GetShootPos();

		if (entityPosition:Distance(playerShootPosition) > 256) then
			self:ForceDropEntity(player);
		end;
	end;
end;
