				...
				if (Clockwork.player:CanAfford(player, cost * amount)) then
					...
					if (player:CanHoldWeight(itemTable("weight") * amount)) then
						...
						if (amount > 1) then
							...
							Clockwork.player:Notify(player, "You have purchased "..amount.." "..Clockwork.kernel:Pluralize(itemTable("name")).." from "..data.entity:GetDTString(0)..".");
						else
							Clockwork.player:GiveCash(player, -(cost * amount), amount.." "..itemTable("name"));
							Clockwork.player:Notify(player, "You have purchased "..amount.." "..itemTable("name").." from "..data.entity:GetDTString(0)..".");
						end;
				...
				if (itemTable) then
					...
					if (data.entity.cwCash == -1 or data.entity.cwCash >= cost) then
						if (player:TakeItem(itemTable)) then
							...
							Clockwork.player:Notify(player, "You have sold 1 "..itemTable("name").." to "..data.entity:GetDTString(0)..".");
						end;
					else
