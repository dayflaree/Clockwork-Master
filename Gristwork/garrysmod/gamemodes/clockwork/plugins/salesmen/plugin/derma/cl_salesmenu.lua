...
function PANEL:Think()
	local function DisplayCallback(displayInfo)
		...
		elseif (self.typeName == "Buys") then
			if (Clockwork.salesmenu:GetBuyRate()) then
		...
		if (Clockwork.config:Get("cash_enabled"):Get()) then
			...
			if (self.typeName == "Sells" and type(Clockwork.salesmenu.sells) == "table") then
				overrideCash = Clockwork.salesmenu.sells[self.itemTable("uniqueID")];
			elseif (self.typeName == "Buys" and type(Clockwork.salesmenu.buys) == "table") then
		...
		if (type(Clockwork.salesmenu.stock) == "table") then
