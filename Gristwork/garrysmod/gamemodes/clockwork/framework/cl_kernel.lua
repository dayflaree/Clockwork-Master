...
Clockwork.datastream:Hook("StopSound", function(data)
	...
	if (Clockwork.ClientSounds[uniqueID]) then
		if (Clockwork.ClientSounds[uniqueID]:IsPlaying()) then
			if (fadeOut != 0) then
...
function Clockwork:ClockworkInitialized()	
	...
	self.ClockworkSplash = Clockwork.kernel:GetMaterial("clockwork/clockwork.png");
...
function Clockwork:CalcViewModelView(weapon, viewModel, oldEyePos, oldEyeAngles, eyePos, eyeAngles)
	if (IsValid(weapon)) then
		local bWeaponRaised = self.player:GetWeaponRaised(self.Client);
		...
		if (!bWeaponRaised) then
			...
			eyeAngles:RotateAroundAxis(eyeAngles:Right(), viewInfo.angles.p);
			eyeAngles:RotateAroundAxis(eyeAngles:Up(), viewInfo.angles.y);
			eyeAngles:RotateAroundAxis(eyeAngles:Forward(), viewInfo.angles.r);
		elseif (self.config:Get("use_free_aiming"):Get()) then
			if (!self.kernel:IsDefaultWeapon(weapon) and !changedAngles) then
				...
				if (!self.SmoothViewAngle) then
					self.SmoothViewAngle = eyeAngles;
				else
					self.SmoothViewAngle = LerpAngle(RealFrameTime() * 16, self.SmoothViewAngle, eyeAngles);
				end;
				
				self.SmoothViewAngle.r = 0;
				
				eyeAngles = self.SmoothViewAngle;
			end;
...
function Clockwork:GetPlayerScoreboardOptions(player, options, menu)
	...
	if (charTakeFlags and self.player:HasFlags(self.Client,charTakeFlags.access)) then
		options["Take Flags"] = function()
			Derma_StringRequest(player:Name(), "What flags would you like to take from them?", player:GetDTString(2), function(text)
...
function Clockwork:RenderScreenspaceEffects()
	if (IN_RENDERVIEW) then return; end;
...
function Clockwork:HUDDrawScoreBoard()
	...
	local bDrawPendingScreenBlack = nil;
	local bDrawCharacterLoading = nil;
	local bHasClientInitialized = self.Client:HasInitialized();
	...
	elseif (!bHasClientInitialized) then
		if (!self.HasCharacterMenuBeenVisible
		and self.plugin:Call("ShouldDrawCharacterBackground")) then
			bDrawPendingScreenBlack = true;
		end;
	end;
	
	if (bHasClientInitialized) then
	...
	elseif (bDrawPendingScreenBlack) then
		self.kernel:DrawSimpleGradientBox(0, 0, 0, scrW, scrH, Color(0, 0, 0, 255));
	end;
...
function Clockwork:PostDrawBackgroundBlurs()
	...
	if (self:PlayerCanSeeDateTime()) then
...
function Clockwork:GetCharacterLoadingTime() return 4; end;
