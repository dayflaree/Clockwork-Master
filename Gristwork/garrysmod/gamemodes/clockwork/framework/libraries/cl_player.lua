...
function Clockwork.player:GetMaxWeight()
	...
	Clockwork.plugin:Call("PlayerAdjustMaxWeight", weight);
...
function Clockwork.player:GetRagdollEntity(player)
	local ragdollEntity = player:GetDTEntity(0);
...
function Clockwork.player:HasAnyFlags(player, flags, bByDefault)
	local playerFlags = player:GetDTString(2);
	...
	if (IsValid(player)) then
		...
		if (sharedVarData) then
			if (sharedVarData.bPlayerOnly) then
				...
				if (class) then
					if (player["GetNW"..class.."2"]) then
						return player["GetNW"..class.."2"](player, key);
					else
						return player["GetNW"..class](player, key);
					end;
				else
					return player:GetNetworkedVar(key);
				end;
