...
if (SERVER) then
	...
	-- A function to get if an entity is in the world.
	function Clockwork.entity:IsInWorld(entity)
		if (IsValid(entity)) then
			local curTime = CurTime();

			if (!self.cwNextCheckInWorld or curTime > self.cwNextCheckInWorld) then
				self.cwNextCheckInWorld = curTime + 1;

				if (self:IsInWorld()) then
					self.bOutOfWorld = false;
				else
					if (self.bOutOfWorld) then
						return false;
					else
						self.bOutOfWorld = true;
					end;
				end;
			end;
		end;
