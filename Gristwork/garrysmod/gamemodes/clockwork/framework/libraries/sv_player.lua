...
function Clockwork.player:CreateCharacterFromData(player, data)
	...
	if (!factionTable.GetName) then
		if (!factionTable.useFullName) then
			if (data.forename and data.surname) then
				...
				if (string.match(data.forename, "([#~%%]+)") or string.match(data.surname, "([#~%%]+)")) then
					return Clockwork.player:SetCreateFault(
						player, "Your forename and surname must not contain special characters!"
					);
				end;
			else
				return Clockwork.player:SetCreateFault(
			...
			if (data.fullName) then
				if (string.match(data.fullName, "([#~%%]+)")) then
					return Clockwork.player:SetCreateFault(
						player, "Your name must not contain special characters!"
					);
				end;
			else
				return Clockwork.player:SetCreateFault(
	...
	if (Clockwork.command:FindByID("CharPhysDesc") != nil) then
		...
		elseif (string.match(data.physDesc, "([#~%%]+)")) then
			return Clockwork.player:SetCreateFault(
				player, "The physical description must not contain special characters!"
			);
		end;
...
function Clockwork.player:SetAction(player, action, duration, priority, Callback)
	...
	if (type(action) != "string" or action == "") then
		...
		player:SetDTFloat(0, 0);
		player:SetDTInt(3, 0);
		player:SetDTString(3, "");
	...
	if (!player.cwAction) then
		...
		player:SetDTFloat(0, curTime);
		player:SetDTInt(3, duration);
		player:SetDTString(3, action);
...
function Clockwork.player:GetAction(player, percentage)
	local startActionTime = player:GetDTFloat(0);
	local actionDuration = player:GetDTInt(3);
	local curTime = CurTime();
	local action = player:GetDTString(3);
...
function Clockwork.player:IsWhitelisted(player, faction)
	for k, v in pairs(player:GetData("Whitelisted")) do
		if (v == faction) then
			return true;
		end;
	end;
...
function Clockwork.player:SetInitialized(player, bInitialized)
	player:SetDTBool(0, bInitialized);
end;
...
function Clockwork.player:GetPhysDesc(player)
	local physDesc = player:GetDTString(1);
...
function Clockwork.player:SetSharedVar(player, key, value)
	if (IsValid(player)) then
		...
		if (!sharedVars or !sharedVars[key]) then
		...
		if (sharedVarData.bPlayerOnly) then
			...
			player.cwNetworkedVars = player.cwNetworkedVars or {};
			...
			if (class) then
				...
				if (player.cwNetworkedVars[key] != value) then
					player.cwNetworkedVars[key] = value;

					if (player["SetNW"..class.."2"]) then
						player["SetNW"..class.."2"](player, key, value);
					else
						player["SetNW"..class](player, key, value);
					end;
				end;
			else
				player:SetNetworkedVar(key, value);
...
function Clockwork.player:GetSharedVar(player, key)
	if (IsValid(player)) then
		...
		if (sharedVarData.bPlayerOnly) then
			...
			if (class) then
				if (player.cwNetworkedVars[key] == nil) then
					return Clockwork.kernel:GetDefaultNetworkedValue(sharedVarData.class);
				end;
			end;

			return player.cwNetworkedVars[key];	
		end;
	end;
end;
...
function Clockwork.player:GetAmmo(player, bDoStrip)
	...
	Clockwork.plugin:Call("GetPlayerAmmo", player, ammo);
	...
	if (player:IsRagdolled()) then
		...
		for k, v in pairs(ragdollWeapons) do
			...
			if (itemTable and Clockwork.plugin:Call("PlayerCanDropWeapon", player, itemTable, NULL, true)) then
				...
				player:TakeItem(info.itemTable);
...
function Clockwork.player:SetBasicSharedVars(player)
	...
	player:SetDTInt(0, player:GetCharacterKey());
	
	if (Clockwork.faction.stored[faction]) then
		player:SetDTInt(2, Clockwork.faction.stored[faction].index);
	end;
