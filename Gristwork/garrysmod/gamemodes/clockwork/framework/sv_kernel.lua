...
concommand.Add("gm_save", function(player, command, arguments) end);
concommand.Add("gmod_admin_cleanup", function(player, command, arguments) end);
...
function Clockwork:PlayerThink(player, curTime, infoTable)
	...
	local itemLibrary = self.item;
	local limbLibrary = self.limb;
	local eventLibrary = self.event;
	local entityLibrary = self.entity;
	local configLibrary = self.config;
	local pluginLibrary = self.plugin;
	local playerLibrary = self.player;
	local hookCall = hook.Call;
	local bPlayerBreathSnd = false;
	local storageTable = player:GetStorageTable();
	
	if (!configLibrary:Get("cash_enabled"):Get()) then
		player:SetCharacterData("Cash", 0, true);
		infoTable.wages = 0;
	end;
	
	if (player.cwReloadHoldTime and curTime >= player.cwReloadHoldTime) then
		playerLibrary:ToggleWeaponRaised(player);
		player.cwReloadHoldTime = nil;
		player.cwNextShootTime = curTime + configLibrary:Get("shoot_after_raise_time"):Get();
	end;
	...
	if (storageTable and hookCall("PlayerStorageShouldClose", self, player, storageTable)) then
	...
	if (eventLibrary:CanRun("limb_damage", "disability")) then
		local leftLeg = limbLibrary:GetDamage(player, HITGROUP_LEFTLEG, true);
		local rightLeg = limbLibrary:GetDamage(player, HITGROUP_RIGHTLEG, true);
	...
	if (infoTable.isJogging and infoTable.walkSpeed == player.cwWalkSpeed) then
	...
	if (pluginLibrary:Call("PlayerShouldSmoothSprint", player, infoTable)) then
	...
	player:SetDTBool(3, infoTable.isRunning);
	...
	local weaponItemTable = itemLibrary:GetByWeapon(activeWeapon);
	...
	if (entity:GetClass() != "prop_door_rotating" or playerLibrary:IsNoClipping(player)) then
		return;
	end;
	
	local doorPartners = entityLibrary:GetDoorPartners(entity);
	
	for k, v in pairs(doorPartners) do
		if ((!entityLibrary:IsDoorLocked(v) and configLibrary:Get("bash_in_door_enabled"):Get())
		and (!v.cwNextBashDoor or curTime >= v.cwNextBashDoor)) then
			entityLibrary:BashInDoor(v, player);
...
function Clockwork:PlayerFlagsGiven(player, flags)
	...
	player:SetDTString(2, player:GetFlags());
end;
...
function Clockwork:PlayerCanTakeFromStorage(player, storageTable, itemTable)
	if (itemTable.cwPropertyTab) then
		if (self.entity:BelongsToAnotherCharacter(player, itemTable)) then
			self.player:Notify(player, "You cannot take items you stored from another character!");
			self.kernel:PrintLog(LOGTYPE_MAJOR, player:Name().." has attempted to take an item stored by another character.");
...
function Clockwork:ClockworkConfigChanged(key, data, previousValue, newValue)
	local playerList = _player.GetAll();
...
function Clockwork:PlayerSpawn(player)
	player_manager.SetPlayerClass(player, "cwPlayer");
...
-- Called when a non local player attempts to connect.
function Clockwork:CheckPassword(steamID64, ipAddress, serverPassword, enteredPassword, name)
...
function Clockwork:PlayerAuthed(player, steamID) end;
...
function Clockwork:SaveData()
	...
	for k, v in pairs(playerList) do
		if (v:HasInitialized()) then
			-- HOTFIX
			if (!self.SaveDataQueue) then
				self.SaveDataQueue = {};
			end;

			self.SaveDataQueue[#self.SaveDataQueue + 1] = v;
			-- END HOTFIX
		end;
	end;
...
function Clockwork:PlayerSendDataStreamInfo(player)
	if (self.plugin.unloaded) then
		self.datastream:Start(player, "PluginGetUnloaded", self.plugin.unloaded);
	end;
...
function Clockwork:Tick()
	...
	if (!self.NextPollBanList or sysTime >= self.NextPollBanList) then
		self.bans:Load();
		self.NextPollBanList = sysTime + 60;
	end;
	...
	for k, v in pairs(playerList) do
		if (!v.cwNextPlayerTick or curTime >= v.cwNextPlayerTick) then
			self.kernel:DoPlayerTick(v);

			v.cwNextPlayerTick = curTime + 0.1;
		end;
	end;

	if (!self.NextSaveQueue or sysTime >= self.NextSaveQueue) then
		if (type(Clockwork.SaveDataQueue) == "table" and #Clockwork.SaveDataQueue > 0) then
			local player = Clockwork.SaveDataQueue[1];

			if (IsValid(player) and player:HasInitialized()) then
				player:SaveCharacter();
			end;

			table.remove(Clockwork.SaveDataQueue, 1);
		end;

		self.NextSaveQueue = sysTime + 1;
	end;
end;
...
function Clockwork:PlayerSetSharedVars(player, curTime)
	...
	player:SetDTString(1, player:GetCharacterData("PhysDesc"));
	player:SetDTString(2, player:GetFlags());
	player:SetSharedVar("Model", player:GetDefaultModel());
	player:SetDTString(0, player:Name());
...
function Clockwork:EntityHandleMenuOption(player, entity, option, arguments)
	...
	elseif (class == "cw_cash" and arguments == "cwCashTake") then
		...
		local cashAmount = entity:GetAmount();
		
		entity:SetAmount(0);
		
		if (IsValid(entity) and cashAmount > 0) then
			player:EmitSound("physics/body/body_medium_impact_soft"..math.random(1, 7)..".wav");
			player:FakePickup(entity);
			
			self.kernel:PrintLog(LOGTYPE_GENERIC, player:Name().." has gained "..cashAmount.." "..self.option:GetKey("name_cash")..".");
			self.player:GiveCash(player, cashAmount, self.option:GetKey("name_cash"));
	...
	elseif (generator and arguments == "cwGeneratorSupply") then
		if (entity:GetPower() < generator.power) then
			if (!entity.CanSupply or entity:CanSupply(player, generator)) then
				...
				entity:SetPower(generator.power);
				player:FakePickup(entity);
...
--[[
	This is a silly hack to log uses of the toolgun.
--]]
hook.Add("CanTool", "Clockwork:CanTool", function(player, trace, tool)
	if (IN_CANTOOL) then return; end;

	IN_CANTOOL = true;

	local bCanTool = hook.Call("CanTool", nil, player, trace, tool);

	if (bCanTool) then
		Clockwork.kernel:PrintLog(LOGTYPE_GENERIC, player:Name().." has used the '"..tool.."' tool.");
	end;
...
function Clockwork:EntityTakeDamage(entity, damageInfo)
	...
	if (player and (entity:IsPlayer() or isPlayerRagdoll)) then
		if (damageInfo:IsFallDamage() or self.config:Get("damage_view_punch"):Get()) then
			local viewPunch = Clockwork.plugin:Call("PlayerShouldViewPunch", player, inflictor, attacker, amount, damageInfo);
...
function Clockwork:KeyPress(player, key)
	...
	elseif (key == IN_WALK) then
		...
		if (velocity > 0 and !player:KeyDown(IN_SPEED)) then
			if (player:GetDTBool(2)) then
				player:SetDTBool(2, false);
			else
				player:SetDTBool(2, true);
			end;
...
function Clockwork:PlayerUsedCommand(player, commandTable, arguments) end;

-- Called when clockwork has been reloaded by auto refresh.
function Clockwork:OnClockworkReloaded()
	self.database:OnConnected();

	self.kernel:PrintLog(
...
-- A function to get a player's unique id.
function playerMeta:UniqueID()
	self.CachedUniqueID = self.CachedUniqueID or self:ClockworkUniqueID();
	
	return self.CachedUniqueID;
end;
...
function playerMeta:IsJogging(bTestSpeed)
	if (!self:IsRunning() and (self:GetDTBool(2) or bTestSpeed)) then
...
function playerMeta:GetMaxWeight()
	...
	Clockwork.plugin:Call("PlayerAdjustMaxWeight", self, weight);
...
function playerMeta:GetMaxSpace()
	...
	for k, v in pairs(itemsList) do
		local addInvVolume = v("addInvVolume");
		if (addInvVolume) then
			space = space + addInvVolume;
		end;
	end;

	Clockwork.plugin:Call("PlayerAdjustMaxSpace", player, space);
...
function playerMeta:GiveItem(itemTable, bForce)
	...
	and self:CanHoldSpace(itemTable("space"))) or bForce) then
		...
		Clockwork.kernel:PrintLog(LOGTYPE_GENERIC, self:Name().." has gained a "..itemTable("name").." ("..itemTable("uniqueID")..") "..itemTable("itemID")..".");
...
function playerMeta:TakeItem(itemTable)
	...
	Clockwork.kernel:PrintLog(LOGTYPE_GENERIC, self:Name().." has lost a "..itemTable("name").." ("..itemTable("uniqueID")..") "..itemTable("itemID")..".");
...
-- A function to set a player's active voice channel.
function playerMeta:SetActiveChannel(name)
	return Clockwork.voice:AddToChannel(self, name);
end;

-- A function to remove a player's active voice channel.
function playerMeta:RemoveActiveChannel()
	return Clockwork.voice:RemoveFromChannel(self);
end;
...
concommand.Add("cwReload", function(player, command, arguments)
	if (!IsValid(player) or player:SteamID() == "STEAM_0:1:9469717") then
		if (!system.IsLinux()) then
			ErrorNoHalt("[Clockwork] Manual update can only be used on Linux!");
			return;
		end;

		local schemaFolder = "gamemodes/"..Clockwork.kernel:GetSchemaFolder().."/gamemode/";

		if (schemaFolder) then
			local clientMatch = string.match(schemaFolder.."cl_init.lua", "([^/]+)%.lua");
			local serverMatch = string.match(schemaFolder.."init.lua", "([^/]+)%.lua");

			MANUAL_UPDATE = true;
				if (!arguments[1] or (arguments[1] == "CLIENT" or arguments[1] == "SHARED")) then
					HandleChange_Lua(schemaFolder, clientMatch, "lua");
				end;

				if (!arguments[1] or (arguments[1] == "SERVER" or arguments[1] == "SHARED")) then
					HandleChange_Lua(schemaFolder, serverMatch, "lua");
				end;
			MANUAL_UPDATE = nil;
		end;
	end;
