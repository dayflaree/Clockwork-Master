	...
	for i = 1, #url do
		...
		if (a < 128) then
			...
			and a <= 64 or a >= 91 and a <= 94 or a == 96 or a >= 123 and a <= 126) then
				output[#output + 1] = "%"..string.format("%x", a);
			else
				output[#output + 1] = c;
			end;
		end;
...
function Clockwork:HandlePlayerJumping(player, velocity)
	...
	if (!player.m_bJumping && !player:OnGround() && player:WaterLevel() <= 0) then
		...
		elseif (curTime - player.m_fGroundTime) > 0 && velocity:Length2D() < 0.5 then
	...
	if (player.m_bJumping) then
		...
		if (player.m_bJumping) then
			player.CalcIdeal = self.animation:GetForModel(player:GetModel(), "normal", "glide");
			return true;
		end;
	end;
...
function Clockwork:HandlePlayerDucking(player, velocity)
	if (player:Crouching()) then
		...
		if (velLength > 0.5) then
			player.CalcIdeal = self.animation:GetForModel(model, weaponHoldType, "walk_crouch");
		else
			player.CalcIdeal = self.animation:GetForModel(model, weaponHoldType, "idle_crouch");
		end;
...
function Clockwork:HandlePlayerSwimming(player, velocity)
...
function Clockwork:HandlePlayerDriving(player)
	if (player:InVehicle()) then
		player.CalcIdeal = self.animation:GetForModel(player:GetModel(), "normal", "idle_crouch")[1];
		return true;
	end;
...
function Clockwork:CalcMainActivity(player, velocity)
	...
	player.CalcIdeal = self.animation:GetForModel(model, weaponHoldType, "idle");
	...
	and !self:HandlePlayerVaulting(player, velocity)) then
		...
		if (player:IsRunning() or player:IsJogging()) then
			player.CalcIdeal = self.animation:GetForModel(model, weaponHoldType, "run");
		elseif (velLength > 0.5) then
			player.CalcIdeal = self.animation:GetForModel(model, weaponHoldType, "walk");
		end;
	end;
	
	if (type(player.CalcIdeal) == "table") then
		if (bIsRaised) then
			player.CalcIdeal = player.CalcIdeal[2];
		else
			player.CalcIdeal = player.CalcIdeal[1];
		end;
	end;
	...
	return player.CalcIdeal or ACT_IDLE, player.CalcSeqOverride or -1;
end;
...
function Clockwork:DoAnimationEvent(player, event, data)
	...
	if (event == PLAYERANIMEVENT_ATTACK_PRIMARY) then
		local attackAnimation = self.animation:GetForModel(model, weaponHoldType, "attack", true);
	...
	elseif (event == PLAYERANIMEVENT_RELOAD) then
		local reloadAnimation = self.animation:GetForModel(model, weaponHoldType, "reload", true);
...
if (SERVER) then
	...
	function Clockwork.kernel:SetupFullDirectory(filePath)
		local directory = string.gsub(filePath, "\\", "/");
	...
	function Clockwork.kernel:DoPlayerTick(player)
		local player = player;
		...
		if (player:HasInitialized()) then
			...
			infoTable.inventorySpace = Clockwork.config:Get("default_inv_space"):Get();
			...
			if (curTime >= player.cwNextSetSharedVars) then
				hook.Call("PlayerSetSharedVars", Clockwork, player, curTime);
				player.cwNextSetSharedVars = nil;
			end;
			
			hook.Call("PlayerThink", Clockwork, player, curTime, infoTable);
		end;
	end;
	...
	function Clockwork.kernel:PerformDateTimeThink()
		--[[ Micro-optimizations ]]
		local timeLibrary = Clockwork.time;
		local dateLibrary = Clockwork.date;
		local pluginLibrary = Clockwork.plugin;
		local defaultDays = Clockwork.option:GetKey("default_days");
		local minute = timeLibrary:GetMinute();
		local month = dateLibrary:GetMonth();
		local year = dateLibrary:GetYear();
		local hour = timeLibrary:GetHour();
		local day = timeLibrary:GetDay();
		
		timeLibrary.minute = timeLibrary:GetMinute() + 1;
		
		if (timeLibrary:GetMinute() == 60) then
			timeLibrary.minute = 0;
			timeLibrary.hour = timeLibrary:GetHour() + 1;
			
			if (timeLibrary:GetHour() == 24) then
				timeLibrary.hour = 0;
				timeLibrary.day = timeLibrary:GetDay() + 1;
				dateLibrary.day = dateLibrary:GetDay() + 1;
				
				if (timeLibrary:GetDay() == #defaultDays + 1) then
					timeLibrary.day = 1;
				end;
				
				if (dateLibrary:GetDay() == 31) then
					dateLibrary.day = 1;
					dateLibrary.month = dateLibrary:GetMonth() + 1;
					
					if (dateLibrary:GetMonth() == 13) then
						dateLibrary.month = 1;
						dateLibrary.year = dateLibrary:GetYear() + 1;
					end;
				end;
			end;
		end;
		
		if (timeLibrary:GetMinute() != minute) then
			pluginLibrary:Call("TimePassed", TIME_MINUTE);
		end;
		
		if (timeLibrary:GetHour() != hour) then
			pluginLibrary:Call("TimePassed", TIME_HOUR);
		end;
		
		if (timeLibrary:GetDay() != day) then
			pluginLibrary:Call("TimePassed", TIME_DAY);
		end;
		
		if (dateLibrary:GetMonth() != month) then
			pluginLibrary:Call("TimePassed", TIME_MONTH);
		end;
		
		if (dateLibrary:GetYear() != year) then
			pluginLibrary:Call("TimePassed", TIME_YEAR);
		end;
		
		local month = self:ZeroNumberToDigits(dateLibrary:GetMonth(), 2);
		local day = self:ZeroNumberToDigits(dateLibrary:GetDay(), 2);
		
		self:SetSharedVar("Minute", timeLibrary:GetMinute());
		self:SetSharedVar("Hour", timeLibrary:GetHour());
		self:SetSharedVar("Date", day.."/"..month.."/"..dateLibrary:GetYear());
		self:SetSharedVar("Day", timeLibrary:GetDay());
	end;
	...
	function Clockwork.kernel:DistributeGeneratorCash()
		...
		for k, v in pairs(generatorEntities) do
			...
			if (IsValid(player) and v:GetPower() != 0) then
				...
				v:SetPower(math.max(v:GetPower() - 1, 0));
	...
	function Clockwork.kernel:AddWorkshopCollection(id)
	...
	function Clockwork.kernel:HandleItemSpawnIconClick(itemTable, spawnIcon, Callback)
		...
		local itemMenu = self:AddMenuFromData(nil, options, function(menuPanel, option, arguments)
			...
			local menuItems =  menuPanel:GetChildren();
			local panel = menuItems[#menuItems];
	...
	function Clockwork.kernel:DrawCinematicIntroBars() end;
...
function Clockwork.kernel:SetSharedVar(key, value)
	...
	if (sharedVars and sharedVars[key]) then
		...
		if (class) then
			...
			if (_G["SetGlobal"..class.."2"]) then
				_G["SetGlobal"..class.."2"](key, value);
			else
...
function Clockwork.kernel:GetSharedVar(key)
	...
	if (sharedVars and sharedVars[key]) then
		...
		if (class) then
			if (_G["GetGlobal"..class.."2"]) then
				return _G["GetGlobal"..class.."2"](key);
			else
