...
SKIN.bg_color 					= Color(236, 240, 241, 255);
...
SKIN.tex.Window.Normal				= Color(231, 76, 60, 255);
SKIN.tex.Window.Inactive			= Color(231, 76, 60, 255);
...
SKIN.tex.CategoryList.Inner		= Color(0, 168, 255, 255);
SKIN.tex.CategoryList.Header	= Color(0, 168, 255, 225);
...
SKIN.Colours.Label = {}
SKIN.Colours.Label.Default				= GWEN.TextureColor(4 + 8 * 8, 500);
...
function SKIN:PaintFrame(panel, w, h)
	...
	if (panel:HasHierarchicalFocus()) then
		self:DrawGenericBackground(0, 0, w, h, self.tex.Window.Normal);
	else
		self:DrawGenericBackground(0, 0, w, h, self.tex.Window.Inactive);
	end;

	self:DrawGenericBackground(0, 21, w, h - 21, self.bg_color);
end;
...
function SKIN:PaintCollapsibleCategory(panel, w, h)
	if (!panel:GetExpanded() && h < 40) then
		surface.SetDrawColor(self.tex.CategoryList.Header);
		surface.DrawRect(0, 0, w, 19);

		return;
	end;
	
	surface.SetDrawColor(self.tex.CategoryList.Inner);
	surface.DrawRect(0, 0, w, 19);
end;
