...
function PANEL:Paint(w, h)
	if (self:GetPaintBackground()) then
		...
		Clockwork.kernel:DrawSimpleGradientBox(0, x, y, width, height, self:GetBackgroundColor());
		
		if (self:IsButton() and self:IsHovered()) then
			Clockwork.kernel:DrawSimpleGradientBox(0, x, y, width, height, Color(255, 255, 255, 50));
		end;
	end	
