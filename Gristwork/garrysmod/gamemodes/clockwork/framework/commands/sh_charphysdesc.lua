...
function COMMAND:OnRun(player, arguments)
	...
	if (arguments[1]) then
		...
		elseif (string.match(text, "([#~%%]+)")) then
			Clockwork.player:Notify(player, "The physical description must not contain special characters!");
			return;
		end;
		
		player:SetCharacterData("PhysDesc", Clockwork.kernel:ModifyPhysDesc(text));
	else
		Clockwork.dermaRequest:RequestString(player, "Physical Description Change", "What do you want to change your physical description to?", player:GetDTString(1), function(result)
