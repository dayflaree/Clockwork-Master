...
Clockwork.datastream:Hook("EditUENotepad", function(player, data)
	if (IsValid( data[1] )) then
		if (data[1]:GetClass() == "cw_uenotepad") then
			if (player:GetPos():Distance( data[1]:GetPos() ) <= 192 and player:GetEyeTraceNoCursor().Entity == data[1]) then
				if (string.len( data[2] ) > 0) then
					data[1]:SetUCText( string.sub(data[2], 0, 500) );
				end;
			end;
		end;
	end;
...
function PLUGIN:LoadNotepad()
	local notepad = Clockwork.kernel:RestoreSchemaData( "plugins/uenotepad/"..game.GetMap() );
	
	for k, v in pairs(notepad) do
		local entity = ents.Create("cw_uenotepad");
...
function PLUGIN:SaveNotepad()
	...
	for k, v in pairs( ents.FindByClass("cw_uenotepad") ) do
	...
	Clockwork.kernel:SaveSchemaData("plugins/uenotepad/"..game.GetMap(), notepad);
end;