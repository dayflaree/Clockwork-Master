	...
	for k, itemList in pairs(items) do
		for k, itemTable in pairs(itemList) do
			if (!itemTable("frequency", false)) then
				break;
			elseif (itemTable:GetData("On") and (!itemTable("factionLock") or itemTable("factionLock")[player:GetFaction()])) then
	...
	if (itemTable) then
		if (string.find(data[1], "^%d%d%d%.%d$")) then
			...
			if (first != "0") then
				...
				Clockwork.player:Notify(player, "You have set this device's frequency to "..data[1]..".");
			else
		...
		if (option == "Set Frequency" and type(arguments) == "string") then
			...
			if (channelTable) then
				if (channelTable.stationaryCanAccess) then
					...
					Clockwork.player:Notify(player, "You have set this bug's listening frequency to "..channelTable.name..".");
				else
					Clockwork.player:Notify(player, "This bug cannot listen in on that channel!");
				end;
			elseif (string.find(arguments, "^%d%d%d%.%d$")) then
				local first = string.match(arguments, "(%d)%d%d%.%d");
				if (first != "0") then
					...
					Clockwork.player:Notify(player, "You have set this bug's frequency to "..arguments..".");
				else
					Clockwork.player:Notify(player, "The frequency must be between 100.0 and 999.9!");
				end;
			else
...
-- For bugs
		...
		local radios = ents.FindByClass("cw_bug");
		...
		for k, radio in pairs(radios) do
			if (player:GetPos():DistToSqr(radio:GetPos()) < range) then
				...
				--[[local data = {
					...
					info.bShouldSend = false;]]
					Schema:AddCombineDisplayLine("(AUDIO MONITOR) "..player:Name()..": "..info.text, Color(225,192,203,255))
