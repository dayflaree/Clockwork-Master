...
local maxChars = PLUGIN.MAX_CHARS;
local PANEL = {};
...
function PANEL:Populate(objectives)
	...
	-- Called each frame.
	function textEntry:OnTextChanged(panel)
		local text = self:GetValue();
		if (string.len(text) > maxChars) then
			self:SetRealValue(string.sub(text, 0, maxChars));
	...
	function button.DoClick(button)
		Clockwork.datastream:Start("EditObjectives", string.sub(textEntry:GetValue(), 0, maxChars));
...
vgui.Register("cwEditObjectives", PANEL, "DFrame");