...
function PANEL:Init()
	if (!Clockwork.theme:Call("PreCharacterMenuInit", self)) then
		...
		self.authorLabel:SetText("DESIGNED AND DEVELOPED BY RED STELLAR.");
		...
		self.createButton:SetText("NEW CHARACTER");
	...
	self.aboutButton:SetText("WEBSITE");
	self.aboutButton:SetAlpha(0);
	self.aboutButton:FadeIn(1);
	self.aboutButton:SetCallback(function(panel)
	 gui.OpenURL("http://www.redstellar.red/index.php")
	end);
	self.aboutButton:SizeToContents();
	self.aboutButton:SetMouseInputEnabled(true);
	self.aboutButton:SetPos((ScrW() * 0.1), ScrH() * 0.7);

				self.aboutButton = vgui.Create("cwLabelButton", self);
	self.aboutButton:SetFont(smallTextFont);
	self.aboutButton:SetText("STEAM GROUP");
	self.aboutButton:SetAlpha(0);
	self.aboutButton:FadeIn(1);
	self.aboutButton:SetCallback(function(panel)
	 gui.OpenURL("http://steamcommunity.com/groups/redstellar")
	end);
	self.aboutButton:SizeToContents();
	self.aboutButton:SetMouseInputEnabled(true);
	self.aboutButton:SetPos((ScrW() * 0.1), ScrH() * 0.75);
		...
		self.disconnectButton:SetPos(ScrW() * 0.1, ScrH() * 0.8);
